...
#     lấy phần trước dấu = đầu tiên → tên pointer domain
...
#   Pointer domains: thêm vào server_name của domain chính (không tạo file riêng)
#
# Usage:
#   sudo bash da_to_openresty.sh
#   sudo bash da_to_openresty.sh --user antiesport
#   sudo bash da_to_openresty.sh --domain antiensport.vn
...
# ── Normalize ssl flag: DA dùng "1","on","ON","yes","YES" → "1" ──
is_ssl_on() {
...
# Format mỗi dòng: subdomain (prefix, không có domain)
# Không đọc từ domain.conf inline nữa — chỉ đọc file này
get_subdomains() {
    local user="$1" domain="$2"
...
# ── Antibot locations (dùng chung cho mọi server block) ───────
antibot_locations() {
    cat << 'LEOF'
    ...
    location = /antibot/debug {
        ...
        content_by_lua_block {
            ...
            ngx.say(string.format(
                '{"version":"4.3","monitor":%d,"challenge":%d,"block":%d}',
...
#   $4 server_names  — space-separated, gồm cả pointers nếu main domain
#   $5 has_ssl       — "1" hoặc "0"
...
server {
    ...
    set \$antibot_debug "0";
    access_by_lua_block { require("antibot").run() }
...
process_domain() {
    ...
    # Normalize ngay: mọi truthy value → "1"
    ...
    # server_name = domain + www.domain + tất cả pointers + www.pointers
    # Pointer nằm trong cùng server block → 1 file duy nhất, không cần file riêng
    ...
    # ── 1 file conf cho domain + tất cả pointers ─────────────
    local content
    ...
    while IFS= read -r sub; do
        ...
        # Cert: dùng cert riêng nếu có, fallback domain chính
        local sub_ssl="0"
...
process_user() {
    ...
    # Xóa toàn bộ conf cũ của user trước khi generate mới
    # Tránh conf của domain đã xóa còn sót lại gây conflict
    if ! $DRY_RUN; then
...
remove_conf() {
    ...
    if [ -n "$domain" ]; then
        ...
        [ -f "${base}/${domain}.conf" ]      && rm -f "${base}/${domain}.conf"      && ok "Removed: ${domain}.conf"
...
install_hooks() {
    ...
    # Hooks rebuild full conf (user + domain scoped)
    local hooks=(
        "user_create_post.sh"
        "user_destroy_pre.sh"
    ...
    # SSL hooks: chỉ cần rebuild conf domain đó rồi reload OpenResty
    # DA gọi ssl_install_post.sh với biến: username, domain
    local ssl_hooks=(
        "ssl_install_post.sh"
        "ssl_delete_post.sh"
    )
...
HOOKEOF
    ...
    # SSL hooks — rebuild domain conf rồi reload OpenResty
    for hook in "${ssl_hooks[@]}"; do
        local hook_file="${hook_dir}/${hook}"
        if [ -f "$hook_file" ]; then
            grep -q "da_to_openresty" "$hook_file" 2>/dev/null && {
                ok "Hook already installed: $hook_file"; continue
...
# ── AntiBot OpenResty SSL sync ────────────────────────────────
_ANTIBOT_SCRIPT="${SCRIPT_PATH}"
_ANTIBOT_LOG="/var/log/antibot_sync.log"
_OR_BIN="/usr/local/openresty/nginx/sbin/nginx"
_ts() { date '+%Y-%m-%d %H:%M:%S'; }
{
  echo "[\$(_ts)] Hook: ${hook} user=\${username:-} domain=\${domain:-}"
  _ARGS="--quiet"
...
$QUIET || {
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  DA → OpenResty AntiBot Config Builder"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
