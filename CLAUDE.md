...
0. **`waf.run_pre`** — runs **before** the cookie fast-path, deliberately. Dispatches `waf/exposed.lua` first (broader, no host gate, cheaper), then `waf/wordpress/paths.lua`. AntiBot scores *who* sends; a WAF inspects *what* is inside — verified identity says nothing about content, so the trust short-circuits must not hide this layer (`cfg.ttl.verified` is 7200s = two hours of uninspected uploads after one PoW at `difficulty="000"`).
   ...
   **Position alone is not enough.** `run_pre` returns `true` only on the block branch; `signal` rules return `false`, so both trust exits in `init.lua` used to swallow `ctx.waf_wp_path` before `compute.lua` read it. Both now carry `not ctx.waf_wp_path`. A WAF signal defeats `verified` but **not** `whitelisted` — whitelist is an explicit operator decision.
   ...
   - `waf/exposed.lua` (3 rules, all `block`, **not WordPress-specific**): `dotfile_exposed` (any path component starting with `.`, one mandatory exception `/.well-known/` or ACME renewal breaks fleet-wide), `dump_exposed` (`.sql .wpress .bak .old .orig .save .swp .swo`, optional `.gz/.bz2/.xz`), `wellknown_exec` (a PHP-executable extension anywhere under `/.well-known/`). Deliberately excludes `.zip`/`.gz` alone — legitimately served from uploads. The ACME exception is mandatory but was **too wide**: exempting the whole tree exempted every `.php` dropped in it. Measured 2026-09-25 on six machines — on disk the entire fleet's real `/.well-known/` content is 23 files (19 `.txt`, 2 extensionless, 1 `.json`, 1 `.html`) and **zero** `.php`; in the logs, thousands of `.php` requests per machine (`gecko-litespeed.php` 2636/2026/1996/113/84/50, `about.php` 4985, `index.php` 1924, plus `wp-conflg.php`, `caches.php`, `classwithtostring.php`), including inside `acme-challenge/` where only tokens belong (`acme-challenge/index.php` 1911) and `/adminfuns.php/.well-known/acme-challenge/file.php` on all six. Real ACME never requests `.php` — a Let's Encrypt token is extensionless base64url. RFC 8615 says `/.well-known/` holds static metadata, so this is a protocol invariant, not a filename list: `security.txt`, `assetlinks.json`, `traffic-advice`, `passkey-endpoints`, `openid-configuration`, `tdmrep.json`, `change-password` all carry real fleet traffic and all still pass.
   - `waf/wordpress/paths.lua` (8 rules): `block` = `wp_upload_exec`, `wp_content_exec`, `wp_includes_exec` (2-entry allowlist), `wp_admin_includes_exec`; `signal` → `ctx.waf_wp_path` = `wp_root_unknown` 0.50, `wp_muplugin_direct` 0.50, `wp_plugin_direct` 0.25, `wp_theme_direct` 0.25.
   ...
   - Tests: `waf/scripts/wordpress_paths_test.lua` (67 assertions, `resty` — PCRE lookahead, `luajit` alone is not enough). `deploy.sh` step `[3b]` gates on it; `SKIP_TEST=1` overrides.
...
| Layer | Path | Role |
|---|---|---|
| core | `core/config.lua`, `redis_pool.lua`, `req_classifier.lua`, `session_richness.lua`, `cookie_registry.lua`, `goodbot_seed.lua`, `proxy_origin.lua`, `data/goodbot.json` | Thresholds, Redis pool, classification, bằng chứng phiên, sổ tên cookie theo host, good-bot registry seed. `proxy_origin.lua` đặt `ctx.behind_proxy` từ **dải IP** Cloudflare (hoặc `SADD waf:proxyhosts <host>`) — chạy TRƯỚC `ip_ban_check` vì với domain sau proxy `ctx.ip` là địa chỉ edge; **không bao giờ đọc giá trị header** (`ctx.ip` là khoá của ban/rate/reputation), header chỉ dùng để phát hiện mạo danh → `ctx.proxy_spoof` |
| waf | `waf/init.lua`, `waf/wordpress/paths.lua`, `waf/exposed.lua`, `waf/scripts/fim.sh` | **Runs before the trust short-circuits** (step 0). `waf/scripts/fim.sh` is the **off-request half of this layer** — a cron-driven file-integrity monitor, deployed with the rest of `antibot-core/` and run from the deployed tree. It exists because a URI WAF cannot see the three commonest paths in: mu-plugins auto-execution (WordPress `include`s every `.php` there on every request — no request to block), LFI, and cron/CLI/direct-to-Apache. It feeds back via `waf:fimnew:<absolute-file-path>` (7d — the key is `document_root .. script_path(uri)` on both sides, NOT host:uri), which **raises** a signal rule's score 0.25/0.50 → 1.0 (= 50 pts, still under CHALLENGE 55) rather than blocking — so `auth_session_cap` still protects a real logged-in admin who legitimately uploaded a file. `run_pre` = access phase, read-only; `run_log` = log phase, owns the only `io.open` and the only WP-host write. Blocks write `ctx.action_reason` themselves. Logs to a separate `/var/log/antibot/waf.log` via `async/waf_logger.lua` — never appended to `antibot.log`, join on `id=`+`ts=`. Columns `exists=` (file really on disk) and `final=` (engine's actual verdict) exist because `status=` is ambiguous: antibot's own PoW page is 200, and WordPress rewrites every missing path to `index.php` so 200 is the *default* for a path that does not exist |
