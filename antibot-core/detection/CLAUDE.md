...
## Update log
- 2026-08-06 — **AhrefsBot vào registry (`ahrefs.net`). Ca THỨ HAI của cùng một lỗi: URL liên hệ và PTR khác TLD.**
  - **Bằng chứng:** 13/13 IP AhrefsBot trong danh sách cấm **vĩnh viễn** có PTR `proxy-*.ahrefs.net` — hạ tầng chính chủ, bị cấm oan.
  - **Cơ chế trượt, giống hệt Amazonbot:** UA khai `+http://ahrefs.com/robot/` ⇒ eTLD+1 = `ahrefs.com`; PTR eTLD+1 = `ahrefs.**net**` ⇒ `ptr_suffix_matches` thất bại ⇒ `fake_good_bot` bot_score 0.85 ⇒ chặn ⇒ leo thang tới cấm vĩnh viễn.
  - **Mẫu hình phổ biến hơn brand gTLD nhiều:** website ở `.com`, hạ tầng crawl ở `.net`. `ahrefsbot = "aggressive"` **đã có sẵn** trong `cfg.rate.good_bot_rate.map` — là code chết cho tới khi có mục registry này.
  - **CÒN TREO — nới `contact_attest`:** nay đã có **hai** ca độc lập (Amazon, Ahrefs) nên việc sửa cơ chế chung là chính đáng. Thiết kế đề xuất: eTLD+1 không khớp thì chấp nhận nếu **nhãn đăng ký trùng** (`ahrefs`.com ↔ `ahrefs`.net; `amazon`.com ↔ TLD `.amazon`) **VÀ** PTR được **xác nhận xuôi** (phân giải tên PTR, bản ghi A phải chứa lại IP nguồn). Vế xác nhận xuôi là bắt buộc — thiếu nó thì chỉ cần đăng ký `ahrefs.xyz` là qua; có nó thì phải kiểm soát reverse DNS của dải IP.
