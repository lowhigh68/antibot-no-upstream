...
## Update log
- 2026-09-15 — **`ip_scope.is_self()` + `cfg.self_addrs`: xu ly ca BA hinh thai mang, khong chi hai.** `$server_addr` mot minh chi du cho hai trong ba: (1) khong NAT, card chi co IP public — cloud171-96 `123.30.171.96`, cloud183-139, cloud28-246; (2) co NAT, card mang CA public lan private — cloud186-126 co `123.30.186.126/25` VA `192.168.186.126/24` cung tren `ens160`, va `$server_addr` la dia chi cua socket DA NHAN ket noi nen tu khop dung duong request di vao. Hinh thai (3) thi khong: **co NAT, card CHI co IP private** — cloud168-101 chi co `192.168.168.101` tren card, con vao/ra deu qua `123.30.168.101`; `$server_addr` va dia chi cua luu luong tu-goi KHONG BAO GIO khop, nen ca hai cong (`ban_store_write` khong tu ket an, `ip_tour` khong tu gan nhan farm) im lang khong kich hoat. `cfg.self_addrs` la danh sach dia chi public khai tay, mac dinh rong — chi hinh thai 3 can dien. **Khong the tu suy ra:** do bang loi goi ra ngoai luc khoi dong thi mong manh va them phu thuoc mang; nhan dien qua UA `WordPress/...` thi gia mao duoc ngay, tuc tu cap quyen mien ban bang mot header. Day la su kien ha tang, cung loai voi `ROOTS` trong `fim.sh`, khong phai mot phong doan. **Chong that bai am tham:** khi `$server_addr` la dia chi RIENG (tuc hinh thai 3) ma `self_addrs` rong, ham ghi mot `ngx.WARN` moi gio (chot bang `antibot_cache:add`, TTL 3600) — vi che do hong o day khong phai bao loi ma la khong lam gi ca, dung lop da giet `wp_paths.mark()` bon thang.
