...
## Update log
- 2026-05-24 — `req_classifier.lua` — **generic auth_endpoint detection + order fix** (security gap):
  - Trước: auth_endpoint chỉ 4 hardcoded WP paths → Joomla/Drupal/Magento/OAuth/custom REST login fall vào navigation (mult 1.0) hoặc interaction (mult 0.6) thay vì auth_endpoint (mult 1.5). Bruteforce protection chỉ hoạt động cho WordPress.
  - Sau: `AUTH_PATH_PATTERNS` table ~30 anchored patterns (WP, Drupal `/user/login`, Magento `/customer/account/loginpost`, generic `/login` `/signin` `/register`, REST API `/api/.+/login`, OAuth `/oauth/token` `/oauth/authorize`, OpenID Connect, Keycloak, 2FA). `args_match_auth()` cover Joomla `option=com_users + task=user.*`, OpenCart `route=account/login`. Chỉ match POST (GET là user mở trang login bình thường).
  - Order fix: auth_endpoint check dời từ sau interaction JSON LÊN ngay sau resource Sec-Fetch-Dest. Bug cũ: POST /wp-json/wp/v2/users với JSON CT match interaction TRƯỚC khi reach auth_endpoint → mult 0.6 thay vì 1.5. Tồn tại từ khi viết classifier.
  - Coverage tăng: site Joomla/Drupal/Magento giờ có bruteforce protection thật. WordPress không regression.
