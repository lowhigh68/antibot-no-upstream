...
- 2026-09-19 — **`proxy_origin.lua` (MỚI) — domain sau Cloudflare: `ctx.ip` là địa chỉ EDGE nên mọi tầng khoá theo IP đang gom hàng trăm khách thật thành một thực thể.**
  ...
  - **Cơ chế:** `ctx.behind_proxy` đặt từ **dải IP Cloudflare công bố** (15 CIDR hardcoded, so sánh số nguyên 32-bit) hoặc khai tay `waf:proxyhost:<host>` (Redis, cache shdict 300s) cho proxy không phải CF. Kích hoạt lại **đúng đường `per_ip_rep`/`SHARED_REP_FACTOR = 0.25`** mà `compute.lua` đã có cho `ip_shared` — không cơ chế mới, giảm chứ không xoá.
