...
## Update log
- 2026-09-19 — **`proxy_origin.lua` (MỚI) — domain sau Cloudflare: `ctx.ip` là địa chỉ EDGE nên mọi tầng khoá theo IP đang gom hàng trăm khách thật thành một thực thể.**
  - **Số liệu buộc phải có tầng này (cloud171-96, `in3mien.com`):** **431 IP edge phục vụ 456 identity**, trong đó **218 lượt có cookie thật** (`rown` 0,65 và 0,80). `rate:<ip>`, `ip_risk:<ip>`, `ext_rep`, `ip_tour` đều tính họ như một. Quy mô: **6 host sau CF trên 171-96, 5 trên 183-139**.
  - **Cơ chế:** `ctx.behind_proxy` đặt từ **dải IP Cloudflare công bố** (15 CIDR hardcoded, so sánh số nguyên 32-bit) hoặc khai tay **`SADD waf:proxyhosts <host>`** (MỘT khoá tập hợp, cache shdict 300s kể cả khi rỗng) cho proxy không phải CF. Kích hoạt lại **đúng đường `per_ip_rep`/`SHARED_REP_FACTOR = 0.25`** mà `compute.lua` đã có cho `ip_shared` — không cơ chế mới, giảm chứ không xoá.
  - **VỊ TRÍ BẮT BUỘC: trước `ip_ban_check`.** Hệ quả trực tiếp: ở đó **không có tín hiệu transport** (`ja3`/`tls13`/`h2` nằm ở bước 11, cuối `STEPS_COMMON`) — đừng dựa thiết kế nào vào chúng. Contract test **[23](c)** gác thứ tự này.
  - **KHÔNG đọc GIÁ TRỊ header — bất biến an ninh, không phải sở thích.** `ctx.ip` là khoá của ban/rate/reputation; tin `CF-Connecting-IP` nghĩa là bất kỳ ai cũng tự khai được danh tính, và kẻ tấn công thuê một CF Worker thì cả điều kiện "ip thuộc dải CF" cũng thoả. Fleet này **cố ý không tích hợp Cloudflare** — tầng này không mở cửa sau cho quyết định đó. Contract test **[23](b)/(b2)**.
  - **Bất đối xứng an toàn:** header CHỈ được làm request bị soi **kỹ hơn**. `ctx.proxy_spoof` = có `CF-Connecting-IP`/`True-Client-IP`/`X-Forwarded-For`/`X-Real-IP` mà địa chỉ **không** thuộc dải nào đã xác minh ⇒ tín hiệu **xấu**, trọng số 30 (MONITOR-level một mình: một số proxy nội bộ của khách chưa khai báo cũng sinh header này, và ở đó FP là operator quên khai chứ không phải tấn công). Đo 19-09: 12 lượt `msmobile.vn` + 2 lượt `no1computer.vn` — hai site KHÔNG dùng CF — mang dấu hiệu này.
  - **`proxy_spoof` KHÔNG bọc `per_ip_rep`:** nó là tín hiệu về CHÍNH REQUEST (một header mâu thuẫn với địa chỉ gửi nó), không phải uy tín tập thể của một IP.
  - **Ba hướng đã BÁC BỎ bằng số liệu, ghi lại để không dựng lại:**
    - **Đổi khoá `identity` sang `fp_light`** — `fp_light = md5(ip + ua + asn + ja3)`, nhưng đo trên tập `banned_id` cho `ja3=-` ở **848/848 lượt**, `ja3` riêng biệt = **1**. `fp_light` thoái hoá về `md5(ip+ua+asn)`, vẫn gộp theo IP edge. Nguyên nhân: `banned_id` thoát ở bước 6, `transport` ở bước 11 — `transport/CLAUDE.md:122` đã ghi sẵn.
    - **Khách thật bị gộp** — KHÔNG xảy ra. Đếm UA/identity **chỉ trong tập có cookie thật** (`rown > 0`): **= 1** trên cả ba domain sau CF. Thước đo "đếm UA bị gộp" mà không lọc cookie thì bot xoay UA cho số y như collapse.
    - **`suachualexus.com` là FP** — SAI. Identity mang 41 UA đó là botnet mạo danh Jetpack: `Jetpack/12.x;_WordPress/6.x;_http://siteNNNNNNNN.com`, **tên miền sinh tự động**, không IP nào thuộc dải Automattic. `identity` gộp chúng lại là **làm đúng việc**; 398/920 = 43% `banned_id` là chặn đúng.
  - **Chưa làm (cố ý):** học tự động "host nào sau proxy" từ lưu lượng — 6 host thì khai tay đủ, và một ngưỡng mới là một ngưỡng phải hiệu chỉnh. Mở rộng khi có proxy khác xuất hiện thật. IPv6 chưa phủ (`is_cloudflare` trả `false`, fail-open).
  - **Bẫy regex đã tránh:** dải CF **không** kiểm được bằng tiền tố chuỗi thập phân — `104.16.0.0/13` trải dài `104.16`..`104.23` nên `104.1[6-9]` vừa bỏ sót `104.2x` vừa bắt lầm `104.160.*`. Chính một regex kiểu đó đã làm đọc sai một phép đo hôm 19-09. Đã kiểm 34 IP thật (16 phải-là-CF gồm cả biên, 18 phải-không-phải gồm `104.160.0.1`, `104.15.255.255`, `172.63.255.255`, `172.72.0.1`): 34/34 đúng.
- 2026-09-15 — **Dia chi cua chinh may: NHAN DANG TU DONG tai cho, khong khai tay.** `$server_addr` mot minh chi du cho hai trong ba hinh thai mang: (1) khong NAT, card chi co IP public — cloud171-96 `123.30.171.96`, cloud183-139, cloud28-246; (2) co NAT, card mang CA public lan private — cloud186-126 co `123.30.186.126/25` VA `192.168.186.126/24` cung tren `ens160`, va `$server_addr` la dia chi cua socket DA NHAN ket noi nen tu khop dung duong request di vao. Hinh thai (3) thi khong: **co NAT, card CHI co IP private** — cloud168-101 chi co `192.168.168.101` tren card, vao va ra deu qua `123.30.168.101`; hai gia tri KHONG BAO GIO khop nen ca hai cong (`ban_store_write` khong tu ket an, `ip_tour` khong tu gan nhan farm) im lang khong kich hoat. **Giai phap: `nginx/deploy.sh` sinh `conf/antibot/self_addrs.txt` TAI CHO tren tung may moi lan deploy** — `ip -4 -o addr show scope global` cho dia chi tren card (hinh thai 1, 2) va `getent ahostsv4 $(hostname -f)` cho dia chi ma hostname tro toi (hinh thai 3; qua NSS nen an theo ca `/etc/hosts` lan DNS, khong can `dig`); loai loopback va `0.0.0.0`. `ip_scope.lua` doc file mot lan luc nap module — moi lan deploy deu reload nginx nen file duoc doc lai, khong can timer, khong cham Redis, khong ton gi o duong request. Phai ghi SAU rsync vi `--delete` se xoa file khong co trong nguon, dung ly do `VERSION` cung nam sau. **Vi sao khong khai trong `config.lua`:** file do deploy CHUNG cho ca dan may, nen khai tay se bat moi may mang danh sach IP cua moi may khac, va may thu sau lai phai sua tay lan nua. **Chong that bai am tham:** file thieu hoac rong => `ngx.WARN` mot lan moi gio (chot bang `antibot_cache:add`, TTL 3600), vi che do hong o day khong phai bao loi ma la khong lam gi ca — dung lop da giet `wp_paths.mark()` bon thang. Hinh thai 1 va 2 van an toan khi thieu file nho `$server_addr`; chi hinh thai 3 mat bao ve.
- 2026-09-15 — **`ip_scope.lua`: them `is_self()`.** Phep kiem "request nay do CHINH MAY NAY phat ra" — `tcp_peer() == $server_addr`. Hai noi dung no: `enforcement/ban/ban_store_write.lua` (khong tu ket an) va `detection/ip_tour.lua` (khong tu gan nhan farm). **So voi `tcp_peer()`, KHONG so voi `ctx.ip`:** `ctx.ip` co the do `real_ip_header` ghi de, va `ctx/init.lua` chi tu choi dia chi NOI BO do header khai — dia chi CONG CONG cua chinh may van lot qua. Ham nay cap MIEN TRU, tuc mot dac quyen, nen phai so voi gia tri khong gia mao duoc. Cung ly do da gom `is_private` ve day: hai ban sao cua mot phep kiem cap dac quyen se lech nhau. Luu y van hanh: may co the co NHIEU dia chi tren mot card (do 14-09 tren cloud186-126: `192.168.186.126/24` VA `123.30.186.126/25` cung tren `ens160`) — `$server_addr` la dia chi cua socket DA NHAN ket noi nen van dung cho tung dia chi, khac han cach lay "dia chi dau tien tren card" da lam mot lenh do bao `0` sai su that.

- 2026-09-13 — **`ip_scope.lua` mới + `ctx.ip` không còn tin header khai địa chỉ nội bộ.** `whitelist.lua` bước 2 cấp đặc quyền mạnh nhất hệ thống (miễn TOÀN BỘ pipeline; tín hiệu WAF thắng `verified` nhưng KHÔNG thắng `whitelisted`), và chú thích cũ của nó tự tuyên bố lý do an toàn: *"RFC1918 không route qua public Internet; không thể spoof từ ngoài."* Câu đó chỉ đúng khi `$remote_addr` là địa chỉ TCP thật. Nhánh `chore/add-spec-kit` thêm `nginx/CF/cloudflare-realip.conf` với `real_ip_header CF-Connecting-IP`, include vào `http{}` toàn cục — `$remote_addr` thành giá trị sao chép nguyên văn từ header, và module realip của nginx **không** kiểm tra địa chỉ thay thế có phải IP công cộng. Chuỗi khai thác: `CF-Connecting-IP: 127.0.0.1` → `ctx.ip="127.0.0.1"` → `lan_internal` → `ctx.whitelisted=true` → thoát ở bước 4, bỏ qua scoring/l7/`ip_ban_check`/detection/PoW/luật WAF. Sửa ở **tầng Lua** chứ không chỉ ở config, vì bất biến này phải đúng dù ai thêm `set_real_ip_from` ở đâu: (1) `ip_scope.lua` giữ **một** bản duy nhất của phép kiểm dải nội bộ — bản `local is_private_lan` trong `whitelist.lua` đã gỡ; (2) `ctx/init.lua` đặt `ctx.ip_tcp` (địa chỉ TCP thật, ba trạng thái — `$realip_remote_addr` rỗng nghĩa là "không có realip", không phải "địa chỉ rỗng") và **từ chối** mọi địa chỉ nội bộ do header ghi đè, ghi `ngx.WARN` + `ctx.ip_spoofed`; (3) `whitelist.lua` bước 2 đọc `ctx.ip_tcp`, không đọc `ctx.ip`. Ghim bằng 4 mục trong `contract_test.lua`. Giới hạn đã biết: IP **công cộng** giả mạo vẫn qua được — đó là bản chất của việc tin header một proxy, và chỗ sửa nằm ở phạm vi `set_real_ip_from`, không ở đây.
...
- 2026-09-07 — **`fingerprint/build_light.lua` — `h2_sig` bị loại khỏi băm `fp_light`. Nó là dấu vân tay của REQUEST, không phải của client.**
  ...
  - **KHÔNG thay bằng `{h2_is_h2, h2_order}`:** `h2_order` chưa được đo là có ổn định trong một kết nối hay không. Đưa vào là lặp lại đúng giả định vừa bị bác bỏ.
  - **NỢ HIỆU CHỈNH đã biết, cố ý chưa trả:** `fp_quality` giờ đếm một trường (`h2_sig`) **không tham gia fingerprint**, và vì H1 luôn nhận sentinel `NO_H2` nên nó **tự động ưu ái H2 hơn H1** một bậc 1/5. Giữ nguyên là quyết định tương thích — gỡ ngay sẽ đổi chính sách chấm điểm cùng lúc với đổi identity, hai thay đổi trộn vào một lần deploy thì không quy được nguyên nhân. Trả nợ đúng thứ tự: (1) tách hẳn *tập tạo hash* khỏi *tập tính quality*, (2) đo phân phối `fp_quality` thật sau khi churn đã hết, (3) hiệu chỉnh lại ngưỡng `FP_QUALITY_THRESHOLD = 0.55` trên số liệu đó — chứ không phải giữ nguyên ngưỡng rồi đổi mẫu số.
  - **Lợi ích kèm:** client chuyển H1 ↔ H2 (Chrome connection coalescing) nay giữ nguyên `fp_light` — đúng FP đã hoãn ở `antibot-core/CLAUDE.md` 2026-05-23 "Defer C'".
  - **Churn H1 KHÔNG phải lỗi, đừng sửa:** 100% churn H1 (302 kết nối, 2 máy) là do `ua` đổi giữa các request trên cùng kết nối keep-alive — bot xoay UA (cloud171-96: 10,1 req/kết nối, 2,47 `fp_light`). `ip` không đổi lần nào ⇒ khoá `conn` lành. `fp_light` đổi theo UA là **đúng**.
  - **⚠ Deploy = `fp_light` đổi một lượt toàn đàn máy:** `sess:` mồ côi, `ban:<fp_light>` hết khớp, rate/burst counter reset. **Giờ thấp điểm.** Kiểm chứng: chạy lại `do_churn.sh`, churn H2 phải tụt từ 26-43% xuống ngang H1 (2-8%). `contract_test` mục 13 gác cả hai đường làm hỏng lại.
- 2026-08-31 — **`goodbot_seed.lua` — TTL làm dấu vết nguồn gốc; `goodbot.json` trở thành nguồn chuẩn thật sự.**
  - **Hai lỗi cùng một dòng.** Bản cũ chỉ ghi khoá CHƯA tồn tại. Hệ quả: (1) **sửa suffix của một bot ĐÃ CÓ trong `goodbot.json` không có tác dụng** — khoá tồn tại ⇒ nhánh `skipped` ⇒ file đổi, Redis đứng yên, không log không lỗi; (2) **gỡ một tên khỏi file không gỡ khỏi Redis** — khoá không TTL, sống mãi. `deploy.sh` phải mọc cờ `--goodbot <tên>` để xoá tay và ghi rõ "đã trả giá 2 lần, ahrefsbot là ca gần nhất".
  - **Cách phân biệt mà không cần khoá đánh dấu: dùng chính TTL.** `TTL = -1` (có khoá, không hạn) → của seed → **ghi đè** bằng nội dung file. `TTL = -2` (chưa có) → ghi mới. `TTL >= 0` (có hạn) → người vận hành đặt tạm → **giữ**, để tự hết hạn. `admin/init.lua` giờ ghi mục thêm tay bằng `SETEX` 7 ngày nên rơi vào nhánh ba; hết hạn thì reload kế tiếp kéo về đúng nội dung git. Cùng nguyên tắc fail-closed của khoá `mon:` bên enforcement.
  - **⚠ ĐỔI HÀNH VI.** Chú thích cũ hứa *"admin override qua redis-cli SET vẫn được giữ"* — **không còn đúng**. `SET` trần không TTL nay bị coi là khoá của seed và sẽ bị ghi đè. Muốn đè tay: `SET goodbot:dns:<tên> "<suffixes>" EX 604800`, hoặc bấm nút trên panel.
  - **Vẫn KHÔNG tự xoá khoá thừa.** TTL phân biệt được "seed" với "admin đặt tạm", nhưng không phân biệt được "seed đời cũ" với "ai đó `SET` tay từ trước khi có quy ước này" — xoá nhầm một bot đang chạy tệ hơn giữ một khoá thừa. Việc đó vẫn để `deploy.sh --goodbot <tên>`.
  - Đọc TTL và ghi đều gom vào `pool.pipeline` (2 round-trip cho cả registry thay vì 41 lượt `safe_get`). Dòng tổng kết nâng từ `ngx.INFO` lên **`ngx.WARN`** để thật sự nhìn thấy trong `error.log`: `written=` (đồng bộ từ file) và `kept_override=` (mục thêm tay còn hạn). Đọc TTL hỏng ⇒ mặc định GHI — ghi đè nhầm một override tạm nhẹ hơn là để nội dung file không bao giờ tới được Redis, đúng lỗi đang sửa.
- 2026-08-08 — **`req_classifier.lua:536` — `skip_layers` gán THAM CHIẾU lên bảng dùng chung ⇒ một cú attest tắt vĩnh viễn cluster+graph cho cả class. Ngủ im 2,5 tháng.**
  - **Cơ chế:** `ctx.skip_layers = config.skip_layers` với `config = CLASS_CONFIG[class]` (bảng cấp module). `detection/bot/init.lua` (`contact_attest` dòng 163-165, `analyzer_attest` 202-204) ghi `skip_layers.cluster/graph = true` cho tier S2.5 ⇒ **sửa thẳng vào cấu hình class**, hết vòng đời worker, không gì khôi phục.
  - **Bằng chứng (cloud168-101, file 2,29 triệu dòng):** `auth_endpoint` **5** attest → cluster xuất hiện trong `top=` **2,69%**; `navigation` **200.686** attest → **0,008%**; `unknown` 32.080 attest → 0,011%. `interaction` = 0 đúng thiết kế (đã tắt sẵn) — dùng làm **điểm chuẩn xác nhận phép đo chạy đúng**.
  - **Bằng chứng quyết định là HÌNH DẠNG, không phải tỷ lệ:** 62 lượt cluster của `navigation` rơi vào đúng **12 phút phân biệt / ~11.000 phút**, thành 3 cụm ngay sau 3 lần worker khởi động lại, rồi **im lặng tuyệt đối 5 ngày**. Bắn 22 lần trong một phút rồi bằng 0 suốt 7.000 phút = công tắc bị gạt, không phải phân bố. Cần hình dạng vì tỷ lệ thô KHÔNG kết luận được: `top=` chỉ ghi 3 tín hiệu đóng góp cao nhất nên vắng mặt ≠ không chạy, và `auth_endpoint` (mult 1,5, đất bruteforce) tự nhiên xếp cluster cao hơn.
  - **Ảnh hưởng:** cluster + graph chết trên **1.409.012 dòng = 62% tổng lưu lượng, 78% ngoài resource**.
  - **Vá tại nơi sinh ra bí danh** (sao chép nông trong `req_classifier`), KHÔNG vá ở hai chỗ ghi — để code tương lai ghi vào `ctx.skip_layers` cũng an toàn.
  - **KHÔNG phải bản vá trung tính.** Nó **bật lại hai tầng chấm điểm cho 62% lưu lượng** sau 2,5 tháng tắt: điểm `navigation`/`unknown` sẽ tăng, một phần vượt ngưỡng monitor/challenge vốn đang không vượt, và thêm chi phí Redis mỗi request. Trọng số `cluster_score`/`graph_flag` chưa từng hiệu chỉnh trên dữ liệu hiện tại. **Deploy RIÊNG, giờ thấp điểm, theo dõi `action=` 30 phút đầu, vọt thì revert** (commit độc lập, đảo một dòng).
- 2026-08-02 — **`fingerprint/asn.lua`: log phụ-thuộc-thiếu nâng `WARN` → `ERR`. Sự cố `resty.maxminddb` không cài ⇒ 4 cơ chế chết âm thầm nhiều ngày.**
  - **Triệu chứng:** file `/usr/share/GeoIP/GeoLite2-ASN.mmdb` (11 MB) **có mặt bình thường**, nhưng module `resty.maxminddb` KHÔNG được cài ⇒ `pcall(require, ...)` fail ⇒ `return nil` ⇒ **`ctx.asn = nil` trên 100% request**.
  - **Chữ ký nhận dạng (bằng chứng runtime, không phải suy diễn):** `grep -oP ' reason=\Kgood_bot\S*' antibot.log | sort | uniq -c` ra **chỉ** `good_bot_verified` (30.281), **0** `good_bot_asn_verified` và **0** `good_bot_asn_lite`. Đường DNS vẫn sống nên phần lớn crawler vẫn qua ⇒ **che mất triệu chứng**.
  - **Bốn thứ chết theo:** (1) S3 `asn_fallback_verify` — Googlebot/Bingbot mất lưới đỡ khi DNS timeout/NXDOMAIN; (2) S3 `lite_verify` cho class `resource` — mọi ảnh/CSS crawler tải đi qua chấm điểm đầy đủ kèm kill-switch resource; (3) `fleet.is_good_crawler` return false ở dòng 71 ⇒ **Google/Bing/Meta/Apple/CocCoc KHÔNG có lá chắn fleet** (đúng rủi ro mà `fleet/trusted.lua` gọi là "SEO disaster"); (4) `fleet.is_trusted` ⇒ dải ISP tiêu dùng VN có thể bị gắn cờ /24.
  - **Vì sao ẩn được lâu:** log báo ở `ngx.WARN`, mà per-domain `error_log` mặc định mức `error` ⇒ lọc sạch. Đây là lần thứ N của cùng một bẫy — xem `memory/feedback_measure_before_conclude.md`. Đã nâng cả hai nhánh (module thiếu + mmdb init fail) lên `ngx.ERR` kèm nêu rõ hậu quả trong chính dòng log.
  - **Khi cài lại `opm install leafo/lua-resty-maxminddb` + reload:** `fp_light = md5(ip|ua|asn|ja3|h2)` ⇒ `asn` đổi nil→thật ⇒ **toàn bộ `fp_light` đổi một lần** (`sess:<fp>` mồ côi, `ban:<fp_light>` hết khớp, counter reset), y như lần bật JA3. Reload giờ thấp điểm, bỏ qua số liệu 1-2 giờ đầu.
