...
| `ssl_client_hello_by_lua_block` ONLY in default_server (`default.conf`) | Chạy tại ClientHello — TRƯỚC khi SNI phân giải → nginx luôn dùng callback của SSL_CTX thuộc default server. Đặt per-domain = **config chết, không bao giờ chạy** (ngoài ra còn gây SNI cert selection bug) | `nginx/default.conf`; `da_to_openresty.sh` KHÔNG sinh nó vào per-domain |
| `ssl_certificate_by_lua_block` (ja3s) thì per-domain là ĐÚNG | Chạy SAU khi SNI đã phân giải | `da_to_openresty.sh` |
...
## Update log
- 2026-07-31 — **JA3 chưa từng chạy: `ssl_client_hello_by_lua_block` đặt sai chỗ** (`nginx/da_to_openresty.sh`). Bộ sinh config đặt handler vào **mọi per-domain block**, ngược với quy tắc đã ghi ở bảng "Cross-cutting rules" phía trên.
