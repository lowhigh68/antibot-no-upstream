...
## Update log
- 2026-05-24 — **Generic auth_endpoint detection + classify order fix** (`core/req_classifier.lua`). Security gap: trước đây chỉ 4 WordPress paths được auth class (mult 1.5). Joomla/Drupal/Magento/OpenCart/OAuth/2FA/custom REST login đều bị classify nhầm (nav mult 1.0 hoặc interaction mult 0.6) → bruteforce protection chỉ hoạt động cho WP. Fix: `AUTH_PATH_PATTERNS` ~30 anchored generic patterns + `args_match_auth` cho Joomla/OpenCart query-string routing. Order fix: dời auth_endpoint check lên trước interaction JSON check (bug: POST `/wp-json/wp/v2/users` với CT application/json match interaction trước → never reach auth check). Chỉ match POST (GET login page là user mở trang bình thường).
