...
- 2026-06-19 (followup) — **Split anti-sharing threshold by scope** (`core/config.lua` + `init.lua` + `core/access/whitelist.lua`). Earlier same-day commit used single `max_ips_per_handle=3` for both cookie and device scopes — FP risk for device scope.
  ...
  - **Calibration of 10 for device**: against observed 43.172/43.173 bot pattern (~35 IPs ÷ 10 UAs ≈ 3-15 IPs per (UA, /16) over 10-min sustained attack). Threshold 10 catches bot within 10-15 min while protecting moderate real-user pools. Tune up (15-30) for high-traffic sites, down (5-7) for low-traffic. Operator monitors `[whitelist] verified_share revoked scope=device shared=N ua=...` log — if revokes are happening on UAs that look like Vietnamese users (Chrome stable), threshold too tight; if revokes are mostly bot UAs, threshold correct.
- 2026-06-19 — **Extend anti-sharing defense to device-canvas path** (`core/access/whitelist.lua` + `core/config.lua` + `init.lua`). Continuation of 43.172/43.173 scraper attack.
  - **Discovery**: After 2026-06-18 cookie defense deployed, scraper continued. Antibot.log on vietship.net showed `18703 reason=device_canvas_verified` vs only `160 reason=score` for `ip=43.17[23]` — 99% bypass via DEVICE path, NOT cookie path. Admin attempt to `redis-cli DEL verified:<id>` returned `(integer) 0` confirming those handles aren't cookie-shaped — they're `verified:device:<device_id>`.
  - **Root cause**: `lookup_device_by_ua` in `whitelist.lua` step 6 binds to /16 subnet (designed for VN mobile carrier roaming UX). Bot accumulates `device_ua:<ua_hash>:<ip16>=device_id` per UA in pool. Once verified, ANY IP in same /16 with same UA matches → `verified:device:<device_id>=1` → `device_canvas_verified`. 35-IP fleet within 43.173.0.0/16 with same UA pool bypasses all detection.
  - **Defense**: Inside `lookup_device_by_ua`, after confirming `verified:device:<device_id>="1"`, atomically SADD source IP into `verified_ips:device:<device_id>` SET (TTL 86400s) + SCARD. If `SCARD > cfg.verified_share.max_ips_per_handle` (default 3) → revoke via DEL `verified:device:<device_id>` + DEL `device_ua:<ua_hash>:<ip16>` + DEL ips set, return nil → request falls through to step 7 earlyid (per-(ip+ua) — naturally bounded) or step 8 onward → re-challenge.
  - **Config refactor**: `cfg.cookie` → `cfg.verified_share` (generic name reflects scope expansion). Field `max_ips_per_cookie` → `max_ips_per_handle`. Backward-compat alias `cfg.cookie = cfg.verified_share` preserved.
  - **Cookie path naming**: Redis key `cookie_ips:<cookie>` → `verified_ips:cookie:<cookie>` for consistency with device scope.
  - **Log**: `[whitelist] verified_share revoked scope=device shared=N handle=<first8> ip=... ua=...` at WARN.
  - **Remaining vectors**: Step 7 `earlyid_verified` (`verified:<md5(ip+ua)>=1`) is per-(ip+ua) → naturally IP-bound, bot must re-solve PoW per new IP for new identity. Step 5 `cookie_verified` already protected by init.lua check_verified_cookie. Step 6 now protected. Bot remaining attack surface: solve PoW per IP per UA = 35 IPs × N UAs = full enumeration → economically unviable.
