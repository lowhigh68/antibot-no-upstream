...
## Update log
- 2026-06-19 (followup) — **Subnet block labels + hit telemetry + admin dashboard panel** (`core/access/subnet_block.lua` + `core/config.lua` + `admin/init.lua`). Operationalize subnet blocklist for ops review.
  - **Schema refactor**: `cfg.subnet_block` entries from bare CIDR strings to `{ cidr = "...", label = "...", note = "..." }`. Backward-compat: bare strings still accepted (label defaults to "default"). Label is operator-defined category (snake_case suggested) — propagates into `ctx.action_reason = "banned_subnet:<label>"` for log grep + dashboard grouping.
  - **Hit telemetry**: each block INCR `subnet_hit:<cidr>:<YYYYMMDD>` (TTL 8d) via `ngx.timer.at(0, ...)` — async, never blocks request. Provides per-subnet per-day visibility for operator to decide which subnets still active vs candidates for removal.
  - **Admin dashboard panel** (`/antibot-admin` tab `🛡️ Subnet Blocks`): shows CIDR + label tag + prefix + hits today + hits 7d + note. JSON endpoint `/antibot-admin/data` adds `subnet_blocks` array with parsed rule list (via `subnet_block.list_rules()`) joined with Redis hit counters.
  - **Common label categories** (operator-defined, not enforced): `scraper_fleet_<region>`, `ddos_pool`, `credential_stuffing`, `ai_crawler_unauth`, `vpn_abuse`.
  - **Seed updated**: `43.172.0.0/15` now labeled `scraper_fleet_cn_hk` with full incident note.
- 2026-06-19 — **Deterministic subnet blocklist** (`core/access/subnet_block.lua` NEW + `core/config.lua` + `init.lua`). Operator-managed CIDR blocklist for subnets empirically confirmed as pure bot infrastructure (no legitimate users mixed in).
  - **Incident driver**: 43.172.0.0/15 attack 2026-06-18 → 19. 44+ distinct IPs in 2-min window, 16+ Chrome versions (including ancient 103/104/105), 100% Windows Chrome (zero platform diversity), 20+ expensive endpoints targeted in parallel, zero verified cookie hits, PHP-FPM saturation. Operator firewall block confirmed: site load returns to baseline immediately and stays normal → proves zero legitimate users mixed in.
  - **Mechanism**: `cfg.subnet_block` is a list of CIDR strings. `subnet_block.lua` parses at module load into pre-computed (base_masked, mask, prefix) tuples. Per request: ip_to_int + single `bit.band` per rule (sub-microsecond, zero Redis). Runs in STEPS_COMMON after ctx_layer.init (needs ctx.ip) and before session_richness/ip_ban_check (skip wasted Redis on confirmed bot). Match → `action=block reason=banned_subnet`, exit 403.
  - **Validation protocol** (in config.lua comment): firewall-block subnet at OS level → observe load → if returns to baseline ≥30 min → add to `cfg.subnet_block` → reload nginx → remove firewall rule.
  - **Seed**: `43.172.0.0/15` (APNIC Chinese/HK delegation — no rationale for VN logistics site traffic).
