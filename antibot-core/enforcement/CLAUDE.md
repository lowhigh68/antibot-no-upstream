...
## Update log
- 2026-09-13 — **`ban_store_write.lua`: khong bao gio tu ban IP cua chinh may.** Do tren cloud186-126: trong 3.604 luot block co `richness>=0.5` ngay 12-09, **2.010 luot den tu `123.30.186.126` — IP cong cong cua chinh may do**, chi 54 luot la nguoi dung that (dai WARP `104.28.237.239`). Do la may chu tu goi minh: wp-cron, kiem tra cap nhat plugin, HTTP giua cac site cung may; tat ca ghi `id=-` vi `ip_ban_check` thoat truoc khi identity duoc tinh. Moi wp-cron tren may an 403 ma khong ai bao. Whitelist KHONG cuu duoc vi `init.lua` xep `ip_ban_check` TRUOC `access_layer` — dung vet bat doi xung da lam `auth_session_cap` khong cuu duoc admin bestcargo.vn hom 12-09 — nen phai chan o cho TAO ban. Phep thu la `ctx.ip == ngx.var.server_addr`: tong quat, khong danh sach ten, khong cau hinh theo may, va khong the gia mao tu ngoai vi client ngoai luon co `remote_addr` khac `server_addr`.

- 2026-09-12 — **Sự cố bestcargo.vn: ban IP không có lối thoát nào cho người đăng nhập.** Một quản trị viên đang đăng nhập lưu bài bằng trình dựng trang — **8 POST `admin-ajax.php`, mỗi cái 113 KB, trong 7 giây** — bị `banned_ip`, 403. Log cho thấy `score=0.0` và `id=-`: request **chưa bao giờ tới engine**. `l7.ban.ip_ban_check` chạy ở bước 2 của `STEPS_COMMON`, **trước** cả cookie fast-path, nên `auth_session_cap` không bao giờ được áp.
  - **Bất đối xứng là gốc.** `l7/ban/ban_store.lua` (ban theo **identity**) đã có "Fix A" — phiên đăng nhập thì bỏ seal, trả về cho scoring phán live. `l7/ban/ip_ban_check.lua` (ban theo **IP**) có **đúng một** lối thoát: UA tự khai là good bot. Người thật không có đường nào.
  - **Không phải lỗi Cloudflare.** IP là `104.28.237.239` (CLOUDFLARENET) nhưng **không có cấu hình `real_ip` nào** và chỉ **1 identity** trên IP đó trong 24 giờ — người dùng xác nhận họ dùng **WARP**. Site không nằm sau Cloudflare. **Tuyệt đối không thêm `set_real_ip_from` cho dải CF ở đây**: nó sẽ cho bất kỳ ai tự khai IP bằng một header và vô hiệu hoá toàn bộ tầng ban + rate limit.
  - **`ip_shared_verified` không bắn, và nó đúng.** Tier-1 cần ≥6 UA khác nhau; Tier-2 (miễn ban IP) cần thêm ≥3 identity có cookie thật. IP WARP đó chở 1 identity trên toàn máy — WARP chia sẻ toàn cầu nhưng với đàn máy này nó là IP một người.
  - **Hệ quả riêng của WARP, độc lập với ban:** WARP **xoay IP**, mà `identity = md5(ip + ua_norm)`. Mỗi lần IP đổi, người dùng thành client hoàn toàn mới — `richness:max:<id>`, lịch sử tin cậy, ban history đều mất. Whitelist IP **không** phải cách sửa bền cho nhóm này.
  - **Sửa ở chỗ TẠO, không nới chỗ KIỂM.** `ban_store_write.lua` nay miễn tạo `ban:<ip>` khi `session_cookie_known == true` **và** `session_richness_own >= 0.5`. Nới `ip_ban_check` thì đối xứng hơn nhưng **làm yếu phòng tuyến rẻ nhất** — ban IP tồn tại để vứt request ở bước 2 khi bị tấn công dung lượng.
  - **Đòi cả hai điều kiện, không chỉ richness.** Gửi 500 byte cookie rác là đạt `richness 0.80`; miễn ban IP chỉ bằng richness sẽ làm một bot xoay UA thành **không thể ban theo IP** — đúng "UA-rotation game" mà Tier-2 sinh ra để chặn. Đòi thêm cookie **do chính host cấp** buộc kẻ tấn công phải lấy cookie thật từ site trước.
  - **CỰC `== true`, không phải `~= false`** — miễn trừ là **trao đặc quyền** nên fail-**closed**; ngược với cổng ở `engine.lua` (lấy đi bảo vệ ⇒ fail-**open**). Cùng một trường, hai cực, mỗi bên chọn sao cho hướng hỏng là hướng an toàn. `contract_test.lua` ghim cả hai.

- 2026-09-12 — **`auth_session_cap` đang gác trên một con số THỪA KẾ.** `ctx.session_richness` là **MAX qua cửa sổ TTL**: `core/session_richness.lua` nâng nó lên từ `richness:max:<id>` trong Redis. Nên một client từng gửi cookie béo **một lần** được cửa tin cậy này che cho **mọi** request về sau, kể cả request không mang trạng thái nào. Một cửa tin cậy gác trên giá trị thừa kế thì không còn là cửa tin cậy. Nay `session_richness.lua` phơi bày thêm `ctx.session_richness_own` (điểm của **chính** request này, trước khi mượn Redis) và cổng đọc trường đó. **Không tăng FP**: admin đăng nhập thật gửi cookie ở mọi request nên `r_own` của họ luôn đủ — phép nâng tồn tại cho client *bỏ* cookie, mà admin thì không bỏ. Ghim bằng 2 phép kiểm trong `contract_test.lua`.
  - **Giới hạn còn lại, ghi rõ để không ai tưởng đã xong:** phép kiểm vẫn chỉ đếm **khối lượng** cookie, không kiểm cookie có thật do site này cấp hay không. Đo 2026-09-12: một trình dò `/cgi-bin/php5` vẫn đạt `richness >= 0.5`. Gốc là `has_auth` trong công thức **chỉ** có nghĩa "có header `Authorization`" — WordPress xác thực bằng **cookie** nên admin WordPress **không bao giờ** nhận 0,3 điểm thưởng đó, và điểm của họ chỉ gồm dung lượng + số lượng cookie, trần 0,80 — **trùng khít** với một scanner gửi 4 cookie rác nặng 500 byte. Đóng thật sự cần học tên cookie mà từng host đã `Set-Cookie`; đó là việc riêng, chưa làm.
