...
- 2026-09-25 — **Ba dòng bóng trên 171-96 là BA request khác nhau, cùng một IP — không phải một request như tôi đoán.** `rid=` khác nhau cả ba: `body_php_code`, `upload_php_ext`, `corr_upload_php_payload`, đều từ `107.161.175.27`. Tức một IP thử ba hình: gửi PHP code trong thân, gửi tên tệp `.php`, rồi lần thứ ba trúng cả hai nên correlation nổ. Hai luật đầu có `mode=enforce` mà vẫn `wact=allow` vì chúng là `action = "signal"`; `would=block` của chúng đến từ correlation. Dân số vẫn 3, vẫn quá nhỏ để promote, nhưng không còn là "một lần".
