...
| `qh` `qw` `qhk` `qwk` `qms` | B3: hàng đợi pool lúc thân này vào — **chỉ thân spill**; xem mục "Vùng mù và hàng đợi soi" |
...
### Vùng mù và hàng đợi soi — B1/B3 (roadmap mục 2, GIAI ĐOẠN ĐO)

Người dùng 27-09: phần dữ liệu mù hoặc chưa xác định chính xác thì **ghi lại, rồi lấy log xử lý** — không đặt ngưỡng hay giả định trước khi có số. Hai phần dưới đây chỉ đo, không đổi phán quyết nào.

**B1 — `body_multipart_incomplete`** (observe, điểm 0). Thân multipart không soi được (`scan` khác `ok`/`empty`) **hoặc** kênh tên tệp dừng trước khi xem hết các part (`fntr` ∈ `body_core.FN_INCOMPLETE` = `n hdr bd bval bmax nb`). `matched=` là lý do: mã `scan`, hoặc `fntr_<mã>`. `len`/`disp`/`ending`/`bdup` không thuộc nhóm này — ở đó mọi part tìm thấy đều đã soi trọn. Thân không phải multipart giữ `body_scan_incomplete`. Chưa giả định "multipart = upload": chính sách theo route (B2) chọn sau, từ `postdeploy.sh` mục 8 (lý do / domain / route / lớp).

**Giới hạn của B1:** nó chỉ báo được chỗ dừng mà bộ quét **tự biết**. Góc nhìn PHP của kênh tên tệp hiện mới phủ tầng tham số Content-Disposition (`php_filename`); các tầng trước đó vẫn đi theo tokenizer của ta, và chỗ lệch ở đó B1 không báo được. Đã ghi lại 27-09 để xử lý sau bằng cùng cơ chế (góc nhìn PHP cho cả chuỗi, kiểm bằng `tools/wafdiff`).

**B3 — đếm lượt soi đang bay trong pool.** `thread_pool` và hàng đợi của nó là của **từng worker**, nên đếm bằng bảng Lua cấp module trong `body.lua`. Khoá là `server_name` (tên đầu của server block đã khớp): mọi alias chung một số đếm, còn Host ngẫu nhiên rơi vào server mặc định thay vì đẻ khoá mới. **Không ghi shared dict** (khoá theo host là thứ kẻ gửi chọn), không cần TTL (bảng sống và chết cùng worker). `pcall` quanh bộ chạy để lượt luôn được trả; lỗi ném ra tính như `spill_thread`. Đã kiểm trên OpenResty thật: `pcall` quanh `ngx.run_worker_thread` vẫn yield đúng với nhiều luồng đồng thời.

| cột `[waf-body]` | nghĩa (`-` = thân không qua pool) |
|---|---|
| `qh=` | số lượt đang bay của CHÍNH server block này trong worker, tính cả lượt này |
| `qw=` | tổng số lượt đang bay trong worker — pool có 2 thread + hàng đợi 128; quá 130 thì `scan=spill_thread` |
| `qhk=` / `qwk=` | KiB đang bay (theo `Content-Length`) của server block / của worker |
| `qms=` | ms từ lúc đưa vào pool tới lúc có kết quả: chờ + đọc + soi |

Chưa có ngưỡng. `postdeploy.sh` mục 9 tính "sẽ vượt" cho K = 1, 2, 4, 8, 16 lượt/server block, cùng `qw` và `qms` lớn nhất. **Khi bật thật** (sau khi có số): server block vượt ngân sách nhận **503 kèm `Retry-After`** (người dùng chọn 27-09), chỉ server block đó — không bao giờ fail-close toàn cục.
...
**V7 (2026-09-26): ba vùng, mỗi cột là TẬP luật.** Dòng `[waf-body]` có `nf=` / `fl=` / `fn=` — tập luật tham số (`a,b` hoặc `-`) khớp trong từng vùng của thân. Mỗi (vùng, luật) còn có dòng `[waf]` riêng:

| cột | target dòng `[waf]` | vùng |
|---|---|---|
| `nf=` | `BODY` | mọi byte CHƯA chứng minh là nội dung tệp — cả thân nếu không phải multipart hoặc `pf≠ok`; kể cả header part |
| `fl=` | `BODY_FILE` | nội dung tệp, CHỈ khi `pf=ok` |
| `fn=` | `MULTIPART_FILENAME` | giá trị `filename`/`filename*`, mọi biến thể |

Bộ quét không chọn giữa các vùng. `registry.region_rule` đổi luật theo vùng (bảng `FILE_REGION`; hôm nay chỉ `../` trong nội dung tệp → `body_file_traversal`, observe), và policy gộp CÙNG luật qua các target bằng max, khác luật thì cộng. `../` trong tên tệp nằm ở cả `nf=` lẫn `fn=` — không bị đếm hai lần. `-` ở cả ba có thể là "sạch" hoặc "chưa soi": đọc `scan=`.

> Các cột `argrule`/`fnm`/`fnrule`/`aorig`/`afld`/`acnt`/`fc` và các mục ngay dưới đây là **trước V7** — giữ để đọc log cũ (`waf.log` giữ 30 ngày).
...
## Kiểm chứng: `tools/wafdiff` và `postdeploy.sh`

**`tools/wafdiff/` — fuzz VI SAI với parser multipart THẬT của PHP (roadmap mục 3).** Sinh thân multipart (dạng chuẩn tắc + đột biến: CRLF/LF/NUL, tiền tố boundary và `\n--B` trong nội dung, header gấp dòng/trùng, nháy đơn/không nháy/`filename*`/khoảng trắng trước `=`, preamble/epilogue, 63/64/65 part, cắt thân ở MỌI byte), cho `php-cgi` đọc làm oracle, và kiểm:

| mã | mức | thuộc tính |
|---|---|---|
| P1 | nguy hiểm | `pf=ok` → mọi giá trị `$_POST` còn NGUYÊN trong bản chiếu (không byte `$_POST` nào bị coi là nội dung tệp) |
| P6 | nguy hiểm | luật trong một giá trị `$_POST` phải có ở `nonfile` |
| P7 | nguy hiểm | luật trong bất kỳ byte PHP thấy phải có ở ÍT NHẤT một vùng (hợp các vùng) |
| P8 | nguy hiểm | luật trong tên tệp PHP đọc ra (`full_path`, TRƯỚC basename) phải ở vùng `filename`; khi `pf=ok` còn phải ở `nonfile` — header không bao giờ là nội dung tệp |
| P5 | nguy hiểm | tên tệp PHP thấy (sau basename) mà `upload.check_filename` bắt thì `up_rule` phải nghiêm trọng ít nhất bằng |
| P9 | lỗi | luật trong nội dung tệp PHP: `pf=ok` → ở vùng `file`; không → ở `nonfile` |
| P3 | lỗi | spill (worker, qua pack/unpack) = memory |
| P2 | thông tin | khoảng tệp của WAF mà PHP không giữ, tách theo lý do: `filename=""` (PHP báo `UPLOAD_ERR_NO_FILE`), hai part tệp trùng tên (PHP ghi đè), còn lại → giữ ca để xem tay |

P7 chỉ hỏi "có vùng nào báo không", nên một luật đúng mà sai vùng lọt qua. Sai vùng lại chính là đường hạ điểm, vì `registry.region_rule` đổi luật theo vùng. P8/P9 hỏi "có đúng vùng không". P5/P8 bỏ sót khi kênh tên tệp **đã khai báo** dừng giữa chừng (`FN_INCOMPLETE`, B1) thì chỉ đếm là thông tin; bỏ sót không khai báo mới là vi phạm.

```bash
bash tools/wafdiff/run.sh                 # trong WSL — N=1500 + quét cắt thân
N=3000 SEED=99 bash tools/wafdiff/run.sh  # 0 sạch · 1 có vi phạm · 2 không chạy được / không bảo đảm
```

**FAIL-CLOSED (review 27-09).** Bản đầu chỉ có `set -u` và ghi oracle vào `/var/tmp/wafdiff-oracle` CỐ ĐỊNH. Khi thư mục đó thuộc user khác, ghi `dump.php` báo Permission denied mà script vẫn chạy tiếp với oracle CŨ và có thể thoát 0. Oracle lại là một file PHP sẽ được thực thi, nằm trong thư mục chung. Hiện nay:
- `run.sh` chạy `set -euo pipefail`, mỗi lần chạy dùng một thư mục `mktemp -d` riêng (0700), mọi bước hỏng thoát 2.
- Nếu có vi phạm, thư mục đó được giữ lại để xem ca lỗi.
- `wafdiff.lua` thoát 2 khi oracle không qua được một thân biết trước kết quả (oracle trả rỗng thì mọi phép kiểm rỗng, và kiểm rỗng ra "0 vi phạm").
- Nó cũng thoát 2 khi PHP không đọc được dù chỉ MỘT ca, hoặc khi chính bộ kiểm lỗi Lua (`resty` trả 1 cho lỗi không bắt, trùng mã "có vi phạm").

CHỈ chạy trên máy dev: nó gọi `php-cgi` với một script PHP, nên tự từ chối trên máy có DirectAdmin, không có file `.php` nào trong repo (oracle được sinh lúc chạy vào thư mục tạm riêng), và nằm ngoài `antibot-core/` nên `deploy.sh` không đưa nó vào cây của nginx. **Bắt buộc chạy trước mọi commit chạm `file_ranges`/`projection`/`scan_disposition_headers`/`php_filename`.**

Công cụ đã được kiểm bằng đột biến:
- Coi field là tệp → P1 + P6. Bỏ quét vùng tệp → P7. `pack` đánh rơi vùng tệp → P3.
- Đẩy nội dung tệp sang `nonfile` → P9 (P7 vẫn 0). Đổi chỗ vùng `file`/`filename` → P8.
- Năm ca hỏng oracle hoặc môi trường đều ra mã 2: php-cgi không in gì, oracle rỗng, oracle hỏng xen kẽ, không ghi được oracle, bộ kiểm lỗi Lua.

**`waf/scripts/postdeploy.sh` — báo cáo CHUẨN sau deploy.** Mọi số liệu tính từ mốc `deployed=` của VERSION, trên MỌI file log ghi sau mốc (kể cả file đã xoay, bất kể tên), in file đã đọc và cửa sổ THẬT. Nằm trong repo để cột log đổi thì báo cáo đổi cùng commit.
...
- 2026-09-25 (`a11e499`, `47c18d1`) — **Khung V2: registry + policy + telemetry. Mặc định không đổi một phán quyết nào.** Lý do có khung: đo trên sáu máy cho `score=0.75` trên **100%** dòng `arg_traversal` (1.165/1.167/1.476/1.349/730) — hằng số tuyệt đối, và cùng một 0.75 cho ra bốn `final=` khác nhau (`challenge`/`allow`/`monitor`/`allow`). Detector nói đúng một câu và nói mãi một câu; quyết định do các tầng khác đưa ra. Ba tín hiệu `waf_arg`/`waf_body_arg`/`waf_upload` phải ở trọng số 0 vì không có cách nào nâng an toàn: nâng bất kỳ số nào thì `page_id,pagename` và botnet cùng được nâng. Đó là lý do **sáu hướng liên tiếp bị loại bỏ** trong phiên — thiếu là **cơ chế**, không phải dân số. **Thang điểm không lẫn nhau:** registry dùng thang riêng cho `ctx.waf_score` (`arg_traversal = 35`), còn cầu tương thích `max_field()`/`ctx.waf_* = 1` vẫn truyền `rule.score` từ **detector tables** (thang `[0,1]`: `wp_root_unknown = 0.50`, `arg_traversal = 0.75`), nên `compute.lua` nhận y nguyên như trước. Không ai parse cột `score=` của `waf.log` (kiểm `wafstat.sh` + `admin/init.lua`), nên đổi thang ở đó an toàn. **`evidence.factor`** là chỗ đặt confidence theo ngữ cảnh: `arg_factor_body()` trả `0.05` khi `spill = true` hoặc `family = multipart`, dựa trên đo 25-09 — nhóm `<multipart>` có **9/9 ca** mang `wpauth=1` hoặc `richness ≥ 0.5` (Magento admin upload ảnh, `cl` 1,1–2,0 MB, `../` nằm trong **nội dung file** đúng như `body_core.lua` đã dự báo), còn `<urlencoded>` 4.041 lượt chỉ 0,15%. **Không đọc `richness` trong factor:** `waf.run_pre` ở `init.lua:200`, `session_richness` trong `STEPS_COMMON` ở dòng 223 — đọc nó ở đây luôn ra `nil`, đúng họ lỗi [[feedback-flag-read-before-write]]. **`0.05` chứ không `0`** — ĐÍNH CHÍNH 25-09: lý do tôi ghi ở đây ("factor 0 làm nhãn biến mất") là SAI. Đọc `policy.lua:162`: `add_labels` được gọi khi `not excepted and action ~= "observe"`, nó không đọc `score` một lần nào. Factor 0 làm điểm bằng 0 nhưng nhãn VẪN vào `state.labels`. Giữ 0.05 bây giờ là một lựa chọn khác: để telemetry phân biệt "luật có nổ nhưng bị hạ" với "luật không nổ", và để một ngưỡng điểm tương lai vẫn thấy một phần nhỏ — không còn là ràng buộc kỹ thuật. **Ca chưa xử lý, ghi rõ:** `34.171.136.165` `richness=0.65` `urlencoded` `cl=94..108` — người thật có phiên, vẫn 35 điểm; chưa gây hại vì giai đoạn này `action=allow`, nhưng đó là ca telemetry phải trả lời. Test sau deploy trên 186-126: policy 38/0, wordpress/paths 127/0, contract 224/0, args 62/0, body 140/0, upload 87/0.
- 2026-09-27 — **`tools/wafdiff` fail-closed và kiểm ĐÚNG VÙNG (review 27-09).** Người dùng chạy thật: 8.000 ca, P1/P3/P5/P6/P7 = 0. Nhưng họ tái hiện được lỗ fail-open của runner: `dump.php` trong `/var/tmp/wafdiff-oracle` báo Permission denied, `run.sh` chỉ có `set -u` nên chạy tiếp với oracle cũ và có thể thoát 0. Sửa bằng `set -euo pipefail`, thư mục `mktemp -d` riêng mỗi lần chạy, mọi bước hỏng thoát 2. Còn **hai đường fail-open nữa** trong `wafdiff.lua`, lần này tự tìm ra:
  - PHP không đọc được một ca chỉ bị đếm là "thông tin".
  - Oracle trả rỗng làm mọi phép kiểm rỗng, nên ra "0 vi phạm".

  Nay mọi ca PHP không đọc được đều ra mã 2. Oracle phải qua một thân biết trước kết quả trước vòng lặp. Lỗi Lua của chính bộ kiểm ra mã 2 (`resty` trả 1, trùng mã "có vi phạm").

  Giới hạn thứ hai người dùng nêu là **P7 kiểm hợp các vùng** nên không bắt "đúng luật, sai vùng". Thêm P8 (tên tệp: phải ở `filename`, và ở `nonfile` khi `pf=ok`) và P9 (nội dung tệp: `file` khi `pf=ok`, `nonfile` khi không). P8 cần tên PHP đọc ra **trước** basename (`full_path`, PHP ≥ 8.1): P7 cũ kiểm tên sau basename, mà basename đã xoá `../`, nên nhánh tên tệp của P7 chưa bao giờ kiểm được gì.

  P2 tách theo lý do. Hai seed × 4.750 ca: 551 ca `filename=""` + 39 ca trùng tên, **0 ca chưa giải thích**; P1–P9 đều 0. Đột biến: nội dung tệp sang `nonfile` → P9 = 122, P7 = 0; đổi chỗ vùng `file`/`filename` → P8. Năm ca hỏng oracle hoặc môi trường đều ra mã 2.
- 2026-09-27 — **Roadmap mục 2, giai đoạn ĐO: B1 (`body_multipart_incomplete`) + B3 (đếm lượt soi đang bay trong pool).** Người dùng duyệt B1 + B3, hoãn B2, và đặt nguyên tắc: phần mù hoặc chưa xác định chính xác thì ghi lại rồi lấy log xử lý; khi bật thật thì trả **503 kèm `Retry-After`**. **B1:** thân multipart không soi được hoặc kênh tên tệp dừng giữa chừng (`FN_INCOMPLETE`) nay đi luật riêng, observe điểm 0, `matched=` là lý do; thân khác giữ `body_scan_incomplete`. Tên luật cố ý không có chữ "upload" — chưa giả định multipart là upload. **B3:** thiết kế nháp dùng `antibot_cache` + TTL; đổi sang bảng Lua trong worker vì hàng đợi `thread_pool` vốn là của từng worker — đếm đúng tài nguyên thật, không ghi khoá theo Host vào shared dict, không có gì để rò qua reload. Năm cột `[waf-body]` mới `qh qw qhk qwk qms`; chưa có ngưỡng — `postdeploy.sh` mục 8 (B1) và mục 9 (B3, "sẽ vượt" cho K = 1…16) đọc chúng. Test chạy thật trong WSL: 936 → 974 phép kiểm. Đột biến trên bản sao, cả sáu bị bắt: bỏ `release()`, bỏ `pcall`, gõ sai một mã `FN_INCOMPLETE`, multipart phát cả luật cũ, khoá theo Host thay vì `server_name`, quên ghi số đếm ở nhánh lỗi.
- 2026-09-26 — **`tools/wafdiff` (roadmap mục 3) và góc nhìn CỦA PHP cho kênh tên tệp.** Fuzz vi sai với `php-cgi`: 7.500 ca (2 seed), ~2.850 ca `pf=ok` — P1/P6/P7/P3 = 0, tức phép chứng minh vùng tệp của V6/V7 khớp PHP trên toàn bộ tập. P5 bắt 26/26 ca cùng một gốc: tên tệp trong NHÁY ĐƠN (`filename='shell.php'`) — `php_ap_getword` hiểu nháy đơn, tokenizer `parse_parameters` thì không, nên PHP lưu `shell.php` còn `up_rule` trượt (lệch thứ tư trong `reference_php_rfc1867`, nay đo được). Sửa bằng CƠ CHẾ chứ không vá chỗ lệch: `php_filename` mô phỏng đúng `php_ap_getword`/`php_ap_getword_conf`/`substring_conf` và thêm tên PHP đọc ra như một BIẾN THỂ nữa của kênh tên tệp. Kênh này là kênh BẮT — thêm góc nhìn chỉ tăng phát hiện; vùng `file` (nơi duy nhất hạ điểm) không dùng nó. Sau sửa P5 = 0 trên cả hai seed.
- 2026-09-26 — **V7: bằng chứng thân request THEO VÙNG, policy quyết định (review 26-09 lần 2, roadmap mục 1).** V4–V6 ép mọi thứ vào MỘT `arg_rule` rồi gắn nhãn `arg_origin`, và mỗi lần chọn luật cho ô đó là một đường hạ điểm kẻ gửi điều khiển được — thứ tự part, định dạng, "luật ngoài tệp luôn thắng" (V6), `ARG_RANK` (f7a4099). V7: `body_core.scan` báo ba TẬP luật (`nonfile_rules`/`file_rules`/`filename_rules`); `init.lua` phát mỗi (vùng, luật) thành một hit với target riêng (`BODY`/`BODY_FILE`/`MULTIPART_FILENAME`); `registry.region_rule` quyết luật theo vùng (bảng `FILE_REGION` — mọi luật tham số phải có dòng, hợp đồng ghim); `policy` gộp CÙNG luật bằng max, khác luật thì cộng. **Vì sao TẬP chứ không một luật mỗi vùng:** với gộp max, "một luật/vùng" làm điểm GIẢM khi thêm nội dung — field `../` + tệp `php://input` = 85; thêm `php://` vào field thì field chỉ còn báo wrapper, trùng luật với tệp, còn 50. Vùng `file` quét TRỌN theo vị trí, không sao chép byte tệp và không cắt 8 KB. Gỡ: `ARG_RANK`, `arg_origin`, `reroute_body_arg`, kênh soi nội dung từng part (cap 8 KB, trạng thái `ct`), trạng thái `stop`, `legacy_fnm`/`fnm`. Gói thread lên V7. Thay đổi hành vi CHỈ ở `wscore` và log (`score_enforcement` tắt, `waf_arg`/`waf_body_arg` trọng số 0): cùng luật ở query + thân 70 → 35 (lỗi đếm hai lần có sẵn từ V2, đo trong WSL); hai luật khác nhau ở hai vùng 50 → 85. Test chạy THẬT trong WSL (OpenResty 1.31.1.1), và kiểm bằng đột biến trên bản sao: bỏ một vùng khỏi `BODY_REGIONS`, bỏ hoặc trỏ sai một dòng `FILE_REGION`, `return` thiếu `up_rule`, policy cộng thay vì max, vùng chỉ báo luật đầu — cả sáu đều bị bắt.
- 2026-09-26 — **`rid=` KHÔNG ổn định trong một request — ghép dòng theo `rid` chưa bao giờ chạy được.** Không cấu hình nginx nào nhắc tới `$request_id`, nên biến không có chỉ mục; khi đó mỗi lần đọc `ngx.var.request_id` nginx gọi lại hàm sinh (`ngx_http_get_variable` → `get_handler`), ra một số ngẫu nhiên mới. `waf_logger` đọc nó riêng cho từng dòng, nên hai dòng của CÙNG một request mang hai `rid` khác nhau. Lộ ra khi lệnh đo `body_php_code` ghép `[waf]` ↔ `[waf-body]` theo `rid`: **0/770** dòng ghép được trên sáu máy, kể cả các cặp chắc chắn cùng request (upload của admin có `php=1`). Tôi chỉ kiểm lệnh đó trên log giả do chính tôi đặt `rid` trùng nhau — bộ kiểm mang sẵn giả định cần kiểm. **Sửa:** `req_id(ctx)` đọc một lần, giữ trong `ctx.rid`. **Hệ quả ngược về trước:** mọi kết luận dựa trên `rid` trước bản này không đứng. Mục 25-09 "ba dòng bóng là BA request" SAI — `would=` là giá trị cấp request và correlation chỉ nổ khi hai luật cùng một request, nên ba dòng `body_php_code`/`upload_php_ext`/`corr_upload_php_payload` cùng `would=block` là MỘT request. Mục `0c` "bảy request (bảy `rid=`)" cũng chưa được chứng minh; quyết định gác theo số nguồn /24 thì vẫn đứng. Log cũ (trước khi deploy bản này) vẫn mang `rid` hỏng: ghép bằng bộ ba `ts`+`ip`+`domain` — `antibot.log` cũng có đủ ba cột đó.
- 2026-09-26 — **Review V6: luật ngoài tệp KHÔNG được che luật mạnh hơn trong tệp.** Bản đầu V6 cho `proj_rule` (luật của phần không phải tệp) **luôn** đè luật của cả thân, nên một field `../` (`arg_traversal`, 35) che mất `php://input` hay `%00` trong tệp (50) — hạ điểm ở policy V2 lẫn cầu cũ (`waf_body_arg` 1,00 → 0,75). Kẻ gửi chọn được, tức đúng loại điều kiện hạ điểm đã mở đường né năm lần trước; test (4a) còn ghim chính hành vi đó. Hôm nay không đổi phán quyết nào (`waf_body_arg` trọng số 0, `score_enforcement=false`), nhưng sẽ thành bypass ngay khi bật. **Sửa:** `ARG_RANK` (null_byte 3 > wrapper 2 > traversal 1, đúng thứ tự `check_args_lower` kiểm) — luật mạnh hơn thắng, ngang nhau thì luật ngoài tệp thắng. `file_content` vẫn đòi phần ngoài tệp **sạch**, nên thân có luật ở cả hai vùng ra `unknown`: để một bản sau reroute thêm wrapper/null-byte cũng không che được luật ngoài tệp. **Không** làm "hai fact độc lập" (hướng review gọi là tốt nhất): policy V2 **cộng** điểm các hit (`policy.lua`, `state.score + score`), nên hai fact đổi cách chấm thân hỗn hợp (35 + 50 = 85 thay vì 50) — đó là một quyết định chấm điểm riêng, cần số liệu. Test: (4a) sửa lại, (4d) NUL thô trong field + `../` trong tệp, `ARG_RANK` phủ mọi mục `args.RULES`, e2e (5) ghim `waf_score 50.00` và cầu `1.0000`. Ghi chú vận hành từ review, đã chép vào chú thích cột log: `afld=`/`acnt=` vẫn đi từ parser cũ, chỉ đọc như phân loại khi `pf=ok`.
- 2026-09-26 — **V6: chứng minh "lần khớp nằm trong NỘI DUNG TỆP" bằng sự CÓ MẶT, theo đúng parser PHP. V5 hỏng theo cả hai hướng.** (a) **Không bao giờ hạ được nhóm nó sinh ra để hạ:** V5 suy `fields_complete` từ `STATUS_RANK` — bảng mức độ để báo cáo — nên `ct` (tệp > 8 KB) đẩy mọi upload lớn ra khỏi đường reroute; và nó đòi `acnt == argrule` trong khi `acnt` chỉ thấy 8 KB đầu tệp. Đo 2 giờ sau deploy trên sáu máy: **12/12** lượt luật tham số trên thân từ phiên WordPress đăng nhập (168-101: 4, 28-246: 8, mỗi máy một nguồn /24) ra `aorig=unknown`, **0** `file_content` trên toàn đàn máy; số lượt khớp đúng số thân `fntr=ct` (4/4, 8/9). Tôi còn viết test ghim `fields_complete = false` cho tệp 9 KB — ghim chính cái lỗi. (b) **Còn mở đường né:** V5 đọc "part này là tệp" bằng tokenizer của mình, lệch PHP (`main/rfc1867.c`) ở bốn chỗ: `filename*` (PHP không biết), Content-Disposition trùng (PHP lấy cái đầu), khoảng trắng trước `=` (PHP không trim key), nháy đơn (PHP hiểu). Mỗi chỗ lệch biến một form field thành "tệp" để payload của nó bị hạ điểm. **Sửa:** bỏ các vùng **chắc chắn** là nội dung tệp ra khỏi thân rồi quét **toàn bộ** phần còn lại (`binary = false`); sạch thì luật chỉ có thể nằm trong tệp. "Chắc chắn" = thân ở **dạng chuẩn tắc** trình duyệt gửi, dạng PHP và mọi parser khác đọc giống nhau: `multipart/form-data; boundary=X` với X là bchars không có dấu phẩy, chấm phẩy, khoảng trắng (PHP lấy boundary bằng **chuỗi con** `strstr`); dòng phân cách đầu nằm ngay byte 1; mỗi part đúng một Content-Disposition `form-data; name="…"` hoặc `form-data; name="…"; filename="…"` (giá trị không chứa nháy kép hay gạch ngược), tối đa một Content-Type; nội dung kết thúc ở `\n--X` **đầu tiên** — quy tắc của PHP, tìm `\r\n--X` là sai vì một `\n--X` trần trong tệp làm PHP cắt tệp ở đó và đọc phần sau thành form field; không byte nào sau `--X--` và CRLF (PHP vẫn tìm tiếp dòng `--X` sau dòng kết thúc). Lệch một byte thì **không chứng minh gì**, cả thân giữ nguyên điểm — không vá từng chỗ lệch, bốn chỗ lệch đã biết là bằng chứng cuộc đua đó không thắng được. Thân chuẩn tắc lấy `arg_rule` từ phần **không phải tệp** trước: luật MẠNH hơn thắng, ngang nhau thì luật ngoài tệp thắng (bản đầu cho form field LUÔN thắng — một đường hạ điểm, sửa cùng ngày, xem mục review phía trên), và NUL thô trong form field giờ mới thành `arg_rule` (V5 chỉ đưa nó ra `afld=`, tức không chấm điểm). Gỡ `fields_complete` khỏi `scan_one_boundary`/`filename_rule` (về 5 giá trị trả về, `[27a]` về 5) và gỡ cổng `STATUS_RANK`; `fields_complete` giờ là `proof == "ok"`. `boundaries_of` kiểm trùng **trước** trần (`A;B;C;D;A` hết `bmax` oan). Giao thức **V6** (16 trường), cột log mới `pf=` là lý do không chứng minh được: `ok ct pre hdr cd dl end n`. Tràn 64 part cũng là "không chứng minh" (`pf=n`) — số đo CPU, nâng khi có số liệu `pf=n` trên upload thật. Test: hơn 30 ca mới trong `body_test.lua` (năm ca review nêu tên, mười một ca lệch parser đọc từ rfc1867.c) và **test đầu-cuối đầu tiên** của đường reroute trong `policy_test.lua` (thân thật → `body_core` → `init.lua` → policy). Không đổi phán quyết nào hôm nay: `body_file_traversal` là observe, cầu `waf_body_arg` vẫn trọng số 0.
- 2026-09-26 — **V5: `fields_complete` — chỉ được hạ một nhóm khi đã CHỨNG MINH không còn form field chưa phân loại.** V4 mở lại đúng loại lỗ nó vừa đóng, và review tìm ra kỹ thuật cụ thể: tệp `photo.jpg` chứa `../` trong 8 KB đầu, còn form field `path` nhồi 9 KB padding rồi mới đặt `../../etc/passwd`. Kết quả với V4: `arg_content = arg_traversal` (thấy `../` trong tệp), `arg_field = nil` (payload nằm sau `MAX_PART_LEN`), nên `arg_origin = file_content` và **toàn bộ request** chuyển sang `body_file_traversal` score 0 — trong khi payload thật nằm trong `$_POST`. **Ba lần liên tiếp** một biện pháp phòng vệ tạo ra chính cái nó phòng: `spill` (kích thước), `multipart` (định dạng), rồi `MAX_PART_LEN` (giới hạn quét). Mẫu chung: mỗi lần tôi suy diễn từ **sự vắng mặt** của một tín hiệu, và sự vắng mặt đó kẻ gửi tạo ra được. **Sửa:** (a) **chỉ cap nội dung tệp, không cap form field** — một form field là dữ liệu văn bản vào `$_POST` và `check_args_lower` vốn đã quét toàn thân một lần, nên quét trọn nó không đổi mô hình chi phí; (b) `fields_complete` là cờ AND qua mọi part và mọi boundary, xuống `false` ở **sáu** đường: tràn `MAX_PARTS`, header bị cắt, thiếu dấu dòng kết thúc, không tìm thấy delimiter, trạng thái parser từ `n` trở lên trong `STATUS_RANK`, và (dự phòng) form field bị cắt nếu mai có ai thêm giới hạn cho nó; (c) reroute đòi `fields_complete == true` ở **hai lớp** — `body_core` trước khi đặt `arg_origin`, và `init.lua` trước khi đổi luật, vì đó là nơi duy nhất hạ một nhóm xuống score 0. `== true` chứ không `~= false`: một `nil` từ bản giao thức cũ phải làm phép kiểm **thất bại**.
- 2026-09-26 — **`arg_origin` so CÙNG MỘT RULE, không chỉ "có rule nào đó".** V4 viết `elseif arg_field then`, nên một thân có `../` trong form field và `php://input` trong nội dung tệp — với `arg_rule = arg_php_wrapper` (luật ưu tiên cao hơn) — sẽ báo `arg_origin = form_field`: **sai nguồn**. Chưa tạo bypass vì chỉ `arg_traversal` được reroute, nhưng telemetry nói về một thứ khác, và nó thành bypass ngay khi reroute thêm wrapper hoặc null-byte. Nay `arg_field == arg_rule`, `fn_rule == arg_rule`, `arg_content == arg_rule`; không khớp nào thì `unknown`. **Giới hạn đã biết:** một chuỗi duy nhất mỗi kênh không biểu diễn được nhiều rule cùng xuất hiện — `unknown` là câu trả lời đúng cho trường hợp đó, và nó giữ nguyên điểm.
- 2026-09-26 — **`binary` theo từng nhóm, không hardcode: NUL thô trong form field là tín hiệu thật.** `scan_part_content` gọi `check_args(chunk, false, true)` cho **cả** hai nhóm — `binary = true` tắt phát hiện byte NUL thô, đúng cho nội dung tệp (một JPEG có byte 0 hợp lệ) nhưng **sai** cho form field. Nay `binary = is_file`. Giới hạn còn lại, ghi rõ: nội dung tệp chứa `php://` hoặc `%00` vẫn phát `arg_php_wrapper`/`arg_null_byte` đủ điểm — chỉ `arg_traversal` có số liệu (9/9 ca Magento) nên chỉ nó được tách; hai luật kia là giới hạn đã biết, không phải bỏ sót.
- 2026-09-26 — **`ct` vào `STATUS_RANK` (mức 6), và số boundary candidate có trần.** `scan_part_content` sinh `ct` khi cắt nội dung part, nhưng `STATUS_RANK` không có tên đó — nên `STATUS_RANK["ct"] or 0` trả **0**, và mọi trạng thái khác (kể cả `stop` mức 1) che mất nó. Với `arg_origin` gác trên trạng thái parser thì một `stop` che `ct` làm một thân bị cắt trông như một thân đã soi trọn: đúng điều kiện để reroute sai. Xếp mức 6 (ngang `hdr`) vì nó là một **vùng chưa soi**, không phải ghi chú về độ dài. Và `boundaries_of` thu mọi boundary khác nhau rồi `scan_one_boundary` quét **toàn thân một lần cho mỗi cái** — chi phí `số boundary × kích thước thân`, tức CPU amplification trên worker thread với `Content-Type` là thứ kẻ gửi đặt. `MAX_BOUNDARIES = 4`, vượt thì đánh dấu `bmax` (mức 7, ngang `nb`) và **không** hạ điểm. Giao thức lên **V5** (14 trường + mã phiên bản = 15) vì `fields_complete` là trường mới; `dec_bool` chứ không `dec`, vì chuỗi `"0"` là **truthy** trong Lua và một phép kiểm `~= false` sẽ mở lại đúng bypass vừa đóng.
- 2026-09-25 — **V4: `arg_origin` — biết lần khớp nằm ở ĐÂU, thay cho việc hạ điểm theo định dạng.** Hai lần liên tiếp tôi mở một đường né rồi phải tự đóng: `spill` (kích thước) rồi `multipart` (định dạng). Bài học: **một thuộc tính chỉ dùng được làm cớ hạ điểm khi kẻ gửi KHÔNG chọn được nó**, và cả kích thước lẫn định dạng đều do kẻ gửi chọn. `arg_origin` khác — nó là kết quả của việc **phân tích**, không phải một khai báo của kẻ gửi. Gốc của lỗ: `_M.scan` gọi `check_args_lower` trên **toàn bộ** thân như một chuỗi phẳng, nên một lần khớp `../` không thể truy về đâu; `filename_rule` đã cắt part nhưng kết quả của nó đi vào `fn_rule`, một trường khác. **Năm nhóm, khác nhau về BẢN CHẤT chứ không về mức độ:** `flat` (không phải multipart, nghĩa cũ) · `form_field` (giá trị vào `$_POST` — **giống hệt** urlencoded, giữ nguyên điểm; đây là nhóm mà bản `multipart` miễn trừ oan) · `filename` (`../` trong tên tệp là nguy hiểm thật, đã có kênh riêng `up_rule`/`fn_rule`) · `file_content` (`../` trong byte của tệp đính kèm — nhóm FP thật, 9/9 ca Magento upload) · `unknown` (là multipart nhưng không quy được: parse dở, hoặc khớp nằm ngoài mọi part — **giữ nguyên điểm**, vì "không biết" khác "không có"). **Ba trường độc lập chứ không một `arg_origin` duy nhất:** một thân có thể có `../` ở **cả** form field lẫn nội dung tệp, và một trường buộc phải chọn một — khi đó **thứ tự part** quyết định chọn cái nào, mà thứ tự part là thứ kẻ gửi điều khiển. Test ca (7) đảo thứ tự part và đòi kết quả y hệt. **`file_content` chuyển sang một LUẬT KHÁC (`body_file_traversal`, observe, score 0) chứ không hạ điểm `arg_traversal`** — vì một `factor` nhỏ **vẫn để lại nhãn** (`policy.lua:162` gọi `add_labels` theo `action`, không theo `score`), nên hạ điểm thì nhóm này vẫn kích hoạt được correlation tương lai; đổi luật là đổi **cả** nhãn, và `body.file_content` không trùng `attack.traversal`. Giao thức **V4** (13 trường + mã phiên bản = 14), chỉ nhận V4: `body_worker` chạy trong VM riêng nhưng nạp từ cùng file trên đĩa, nên hai bên chỉ lệch trong khoảng giữa hai lần reload của một lần deploy — ở đó `bad_payload` là câu trả lời đúng vì nó **có tên** và chỉ ảnh hưởng thân đã spill. `MAX_PART_LEN = 8192`: một payload traversal thật dài vài trăm byte (botnet đo được 77–242), còn 8 KB đầu của một tệp thật là header định dạng nơi `../` không xuất hiện tự nhiên. Số liệu dẫn tới thiết kế này: **`fnm=0` trên 36/36 lượt** `arg_traversal`+`arg_null_byte` của 168-123 — không một lượt nào trong tên tệp, nên nhóm `file_content`/`form_field` là nhóm có dữ liệu thật; còn `nargs=-` trên **100%** lượt multipart cả hai máy nghĩa là log cũ **không** phân biệt được hai nhóm đó, nên tỉ lệ giữa chúng chỉ đo được sau khi V4 chạy. Cột log mới: `aorig=` `afld=` `acnt=`.
- 2026-09-25 — **`decide()` tách HAI TRỤC độc lập, và `reason` là `nil` khi không chặn.** Bản trước trộn chúng: `reason` ưu tiên `enforce_candidate` rồi **rơi về** `candidate`, còn `rule_mode` thì **luôn** lấy từ `candidate`. Hai hệ quả sai cùng lúc — (a) `action = "allow"` vẫn mang một `reason`, nên đọc log thấy một request không bị chặn nhưng có lý do chặn; (b) `reason = dotfile_exposed` (từ enforce candidate) đi cùng `rule_mode = shadow` (từ một candidate **khác**, mạnh hơn nhưng ở chế độ bóng) — hai giá trị mô tả **hai rule khác nhau** trên cùng một dòng. Hôm nay chưa sai vì mọi luật đều `enforce` nên hai candidate trùng nhau; nó sai đúng lúc có luật shadow mạnh hơn, tức **đúng lúc bắt đầu promotion** và đúng lúc cần đọc log chính xác nhất. Quy ước mới: `reason`/`rule_mode` chỉ mô tả candidate tạo ra phán quyết thật, và khi `action = "allow"` thì cả hai là `nil`; bằng chứng chế độ bóng nằm ở nhóm `would_action`/`would_reason`/`would_rule_mode`. Nhờ vậy không còn cách nào đọc lẫn hai trục — **một `reason` tồn tại nghĩa là một request thật sự bị chặn**. Điều đó cũng sửa một nhóm counter: `decision:allow:<rule>` trước đây được ghi cho request **không** bị chặn, tức một nhóm counter nói về thứ khác với tên của nó. Mô phỏng ba ca trước khi áp, gồm ca shadow điểm cao hơn enforce — ca duy nhất phân biệt được hai thiết kế.
- 2026-09-25 — **Khung V2 đo xong đợt đầu: ~5.700 lượt chặn thật, 0 phiên đăng nhập, năm máy.** `wp_includes_exec` 4.400+176+68+59, `dotfile_exposed` 1.070+702+329+48+1, `wp_content_exec` 899+167+45+30, `dump_exposed` 281+197+153+17, `wellknown_exec` 230+10+5 — mọi dòng `wact=block` đều `0 auth`. Với dân số này thì kết luận chắc, **không cần chờ 24–48 giờ** như tôi đề xuất ban đầu: 17.147 dòng trong 2 giờ nghĩa là mọi luật trừ correlation đã vượt ngưỡng n≥30 từ phút đầu. Con số "24–48 giờ" tôi chọn theo quán tính, không theo tốc độ dữ liệu thật.
- 2026-09-25 — **Mục `0c` giờ gác trên SỐ NGUỒN (/24), không chỉ số lượt — vì ngưỡng 30-lượt lặp lại đúng lỗi `min_names`.** Đo trên 171-96: bảy dòng `wact=allow would=block` thuộc **bảy request khác nhau** (bảy `rid=`, nên phép đếm lượt không sai) nhưng **cả bảy từ MỘT IP** — `107.161.175.27`, một kẻ tấn công thử nhiều hình. Đếm lượt thì thấy "bốn luật, mỗi luật 1–2 lượt"; đếm nguồn thì thấy "một nguồn duy nhất", và đó mới là thứ quyết định: một mẫu từ một nguồn không nói gì về dân số người dùng thật, **dù có bao nhiêu lượt**. Cùng lỗi đã ghi trong [[feedback-mature-not-representative]] — `min_names` đo số lượng chứ không đo tính đại diện — và ngưỡng 30-lượt tôi đặt trước đó lặp lại chính nó. Nay hai ngưỡng: `MIN_N = 30` (điều kiện số học để một tỉ lệ 0% có nghĩa) và `MIN_SRC = 5` /24 khác nhau (điều kiện đại diện). Đếm /24 chứ không đếm IP: đổi IP trong cùng dải vẫn là một nguồn. **ĐÍNH CHÍNH 26-09: "bảy request" chưa được chứng minh — `rid=` không ổn định (xem mục `rid=` 26-09); quyết định gác theo số nguồn vẫn đứng.**
- 2026-09-25 — **Thêm cột `vfy` vào `wafstat.sh`: cột `auth` trước nay là một SÀN, không phải giá trị thật.** `vfy=1` nghĩa là client có cookie `verified` còn hạn nên nó **thoát fast-path trước `session_richness`**, và dòng log ra `richness=-`. Phép đếm `auth` xếp nó vào nhóm "không phải auth" — sai, vì một client verified là một client đã qua thách đố PoW. Tách `vfy` ra thay vì gộp vào `auth`: gộp sẽ thổi phồng con số FP (một bot đã giải PoW cũng có `vfy=1`), bỏ qua thì con số thấp hơn thực tế; hai cột đọc cạnh nhau mới nói đủ. Dòng `wact=block` có `vfy` khác 0 giờ báo `-> XEM KY` thay vì "0 phiên đăng nhập" — thông tin mà bản cũ mất.
- 2026-09-25 — **`scan=empty` xác nhận đã về nhịp lấy mẫu.** Đo trên 171-96 sau deploy 22:03: `377 scan=empty smp=1` (log trước patch) so với `4 scan=empty smp=20` (sau patch) — `empty` trước đây **không thể** mang `smp=20`, nên bốn dòng đó là bằng chứng. Và `body_scan_incomplete` 263 lượt/100 auth trên máy đó thuộc hoàn toàn về log lịch sử: dòng cuối cùng là **20:56**, không một dòng nào sau mốc deploy. Số `38% auth` đọc được trong `wafstat.sh` là của bản cũ, sẽ biến mất khi log xoay vòng — một lời nhắc rằng `wafstat.sh` đọc cả file nên mọi phép đo hiệu quả của bản vừa deploy phải kèm mốc thời gian, đúng như [[feedback-log-rotate-window]] đã ghi.
- 2026-09-25 — **`waf.configure()` giờ ĐƯỢC GỌI ở production, trên MỌI worker.** Trước đó không caller nào ngoài test gọi nó, nên toàn bộ cơ chế per-domain/exception/profile chỉ **tồn tại trong mã nguồn** — một lớp cấu hình trông như đã chạy. Thêm `waf/runtime_config.lua` (file người vận hành sửa, đi cùng repo nên `rsync --delete` đồng bộ được — khác `VERSION`/`self_addrs.txt` là dữ liệu per-máy phải ghi sau rsync) và nạp nó trong `init_worker`. **Mọi worker, không chỉ worker 0:** `waf/init.lua` giữ `compiled_config` ở cấp module, tức một bản riêng trong từng tiến trình worker — gọi ở worker 0 thì các worker khác vẫn chạy mặc định và lưu lượng chia đôi giữa hai chính sách, một lệch âm thầm không đọc được từ log. **Không** dùng `ngx.timer.at`: `configure()` chỉ đọc một bảng Lua, defer nó sẽ để một khoảng đầu đời worker chạy bằng mặc định. `pcall` vì file đó người vận hành sửa — một dấu phẩy thiếu không được làm nginx không khởi động được, và khi nó hỏng thì WAF chạy mặc định (an toàn hơn bản cấu hình mới theo định nghĩa). Bản đang deploy trùng `DEFAULT` nên **không đổi một phán quyết nào**; `policy_test.lua` ghim điều đó (`mode = enforce`, `score_enforcement = false`, không exception nào) để một lần đổi thật phải là đổi có ý.
- 2026-09-25 — **`scan = "empty"` không còn là `notable`, nên thân rỗng trở lại nhịp lấy mẫu 1/20.** Việc gỡ nó khỏi phát luật (ở trên) chưa đủ: `waf_logger` tính `notable = … or (b.scan and b.scan ~= "ok")`, nên **mọi** thân rỗng vẫn ghi dòng `[waf-body]` 100% thay vì 1/20 — I/O thừa và nhiễu log, đúng thứ cột `smp=` sinh ra để tránh. Với 247 lượt/2 giờ trên một máy thì đó là chi phí thật. Lý do trong chú thích cũ ("một lần không soi được là thông tin") vẫn đúng cho `spill_*`; `empty` không thuộc loại đó. Gỡ khỏi **tập notable**, không gỡ khỏi log: thân rỗng vẫn ra dòng khi trúng nhịp mẫu, và vẫn đếm trong `waf:v2:scan:empty`.
- 2026-09-25 — **Cột độ trễ trước nay là 0 vì PHÉP ĐO KHÔNG HỀ CHẠY, không phải vì WAF nhanh.** Hai lỗi cộng lại. (a) `ngx.now()` trả thời gian **được cache theo request** trong OpenResty — nó không đổi trong suốt một request trừ khi gọi `ngx.update_time()`; và `telemetry.start()` lẫn `telemetry.finish()` đều chạy trong `run_pre`, cùng một request cùng một phase, nên `finish - start` bằng **đúng 0, luôn luôn**. (b) Ngay cả khi đồng hồ chạy, `math.floor(elapsed_ms + 0.5)` làm tròn về ms nguyên, mà `run_pre` là vài phép khớp regex trên URI — mọi mẫu thành 0. Nay `now()` gọi `update_time()` và khóa đổi sang `latency_us_sum` (microgiây), tên mới chứ không giữ tên cũ với đơn vị mới: một khóa cũ còn sót trong shm sẽ bị cộng lẫn với giá trị đơn vị khác và không ai phát hiện. `snapshot()` đọc cả hai khóa nên một máy chưa deploy không hiện ra "không có số liệu". **Đây là điều kiện để trả lời câu hỏi hot-key** (~5 `shared_dict:incr` mỗi request, và `incr` khóa toàn dict chứ không khóa một khóa): chưa tối ưu gì, vì con số cần để biết có đáng tối ưu hay không thì chính nó đang không đo được — đo trước, sửa sau.
- 2026-09-25 — **`telemetry.snapshot()` đọc theo DANH SÁCH KHÓA BIẾT TRƯỚC, không quét dict.** Bản đầu gọi `get_keys(max)` rồi lọc tiền tố *sau* khi quét — hai lỗ: (a) `antibot_cache` dùng chung với dữ liệu vận hành khác, nên nếu nó có hàng chục nghìn khóa thì 2.048 khóa đầu **có thể không chứa một counter WAF nào**, và endpoint trả `summary` toàn 0 kèm `truncated = true` — một phép đo trả về 0 vì *không đo được*, không phải vì bằng 0; (b) `get_keys` **khóa toàn bộ dict** trong lúc quét, và `?max=0` thì quét hết. Tập khóa của module này hữu hạn và biết trước: counter cố định, cộng `rule:<id>`/`excepted:<id>`/`shadow:<id>`/`decision:<a>:<id>` với id lấy **từ registry**, cộng `action:<a>`/`would:<a>` với `a` thuộc bốn giá trị. Đọc thẳng bằng `dict:get()` — không khóa dict, không phụ thuộc dict lớn bao nhiêu. `host:<host>:action:<a>` là ngoại lệ duy nhất (sinh theo dữ liệu) nên chỉ nhóm đó quét, và chỉ khi `per_host` bật; `scanned`/`truncated` giờ **chỉ** nói về nhóm đó. Giá của tập cứng là `SCANS` có thể thiếu một lý do mới thêm vào `body.lua` — ghim bằng hợp đồng mới trong `contract_test.lua`, và hợp đồng đó phải quét **hai dạng** vì `spill_thread`/`nothread` đi qua biến `reason` chứ không phải chuỗi trực tiếp: một phép kiểm chỉ tìm `unscanned%(...)` sẽ báo xanh trong khi bỏ sót đúng hai tên. Hai counter latency giờ cũng đếm `write_errors` — trước đó chúng không truyền `stats`, nên một `antibot_cache` đầy làm chúng thất bại im lặng trong khi bộ đếm lỗi vẫn báo 0.
- 2026-09-25 — **`config.validate()` chạy HAI LƯỢT: bản raw người vận hành viết, rồi bản đã compile.** Lỗ: `compile()` làm `out.exceptions = {}` rồi `append(...)`, và `append` **bỏ qua** giá trị không phải table — nên `exceptions = "invalid"` biến thành `{}` và vượt qua validator, fail-silent ở đúng chỗ vừa làm fail-loud. Cùng thế với mảng thưa (`{ [1]=…, [3]=… }`): `#src` dừng ở lỗ trống đầu tiên nên phần tử thứ 3 bị bỏ lặng lẽ. `validate_raw` đọc bản gốc mà `compile()` giữ lại dưới khóa `__raw`. **Khóa phải là CHUỖI, không được là bảng** — `copy()` làm `out[copy(k, seen)] = …` nên một khóa bảng bị *sao chép* thành bảng mới, và `resolve()` (gọi `copy(compiled)`) trả về bảng không còn khóa nào bằng `RAW_KEY`, khiến `validate()` trên bản resolved bỏ qua lượt kiểm raw trong im lặng. Và `__raw` **không** nằm trong `SCOPE_KEYS` — `reject_unknown` bỏ qua nó riêng: đưa vào allowlist thì nó thành khóa hợp lệ cho người viết cấu hình, và một `__raw = {…}` trong domain scope sẽ được `merge` ghi đè lên bản raw thật, tức lượt kiểm raw đọc một bản do kẻ viết cấu hình chọn. **Giữ chữ ký một tham số** thay vì `validate(compiled, runtime)`: bản hai tham số sạch hơn nhưng làm chín nơi gọi hiện có *vẫn chạy* mà bỏ qua lượt kiểm mới — một tham số tuỳ chọn không bao giờ bắt ai thêm nó vào. Thêm ba phép kiểm: `family` của exception phải có thật trong registry (một họ viết sai làm exception im lặng vô tác dụng), exception phải có **ít nhất một selector** (`id` chỉ là tên truy ngược, nên một exception chỉ có `id` khớp MỌI luật — cách dễ nhất để tắt toàn bộ WAF bằng một dòng trông vô hại), và `domains` không được lồng trong domain scope (`resolve()` không bao giờ đọc nó).
- 2026-09-25 — **Admin đọc cấu hình ĐANG CHẠY, không phải mặc định.** `render_waf_v2` gọi `config.defaults()` để biết dict/prefix của telemetry — đúng khi chưa ai gọi `waf.configure()`, nhưng sai ngay khi có: nó đọc một dict khác với dict đang được ghi và báo "không có sự kiện". Thêm `waf.active_config()` trả bản **sao** của `compiled_config`. Và chặn `?max=0` ngay ở admin: `get_keys(0)` quét toàn dict.
- 2026-09-25 — **`multipart` cũng đã gỡ: `arg_factor_body` giờ không hạ điểm của nhóm nào. Đây là lỗ tôi tự tạo ra khi vừa bịt lỗ `spill`.** Review chỉ ra và nó đúng: `body_core.lua:616` quét **toàn bộ** thân multipart và không phân biệt lần khớp nằm trong nội dung tệp, tên tệp, header part, hay **một form field thường**. Kẻ gửi chỉ cần đặt `path=../../etc/passwd` thành một text part hợp lệ trong `multipart/form-data` — PHP vẫn nạp vào `$_POST` y như urlencoded — và điểm traversal từ 35 xuống 1,75. Lập luận cũ của tôi ("kẻ gửi không đổi được định dạng mà không đổi bản chất request") **sai**: đổi định dạng không đổi việc tham số vẫn tới `$_POST`. Và lỗ này áp cho cả `arg_php_wrapper` và `arg_null_byte`, không riêng traversal. Chưa gây bypass hôm nay (`score_enforcement = false`, cầu cũ ở trọng số 0) nhưng thành bypass đúng lúc ai đó bật một trong hai — loại lỗ ngủ, khó tìm nhất. **Hai lần liên tiếp tôi mở một đường hạ điểm bằng một điều kiện kẻ gửi điều khiển được**: `spill` là kích thước, `multipart` là định dạng. Bài học không phải "chọn thuộc tính khác" mà là: một thuộc tính chỉ dùng được làm cớ hạ điểm khi kẻ gửi **không** chọn được nó, và cả kích thước lẫn định dạng đều do kẻ gửi chọn. Đổi giá có ý: 9 ca FP `multipart` trở lại 35 điểm, vẫn `action=allow` vì trọng số 0, `auth_session_cap` vẫn che. **Hướng đúng, chưa làm:** scanner trả thêm `arg_origin = file_content | form_field | filename`, chỉ hạ `file_content`; `fn_rule` đã làm được nửa việc nhưng hiện chỉ phục vụ telemetry. Việc đó đổi `body_core.lua` + giao thức pack/unpack nên là một commit riêng.
- 2026-09-25 — **`scan = "empty"` không còn phát `body_scan_incomplete`: thân rỗng không phải vùng mù.** Đo trên năm máy sau hai giờ: luật này chiếm **1.353/2.236** dòng V2 trên 186-126 và **2.160/3.827** trên 28-246 — hơn một nửa toàn bộ số liệu của khung. Bóc theo `matched=` trên 171-96 thì **247/247** lượt là `empty`, không một lượt `spill_*` nào. `empty` là nhánh `get_body_file()` trả `nil` (`body.lua:113`): thân request **rỗng thật**, `len = 0` — một POST `api_callback` không có thân là chuyện bình thường (admin-ajax đặt tham số ở query string), và đó là 215/247 lượt. Gộp "không có gì để soi" với "có mà soi không nổi" vào một luật thì luật đó không trả lời được câu nào: không đọc được `body_scan_incomplete` để biết còn vùng mù bao lớn. Cùng họ lỗi với `exists=1` báo sai cho mọi lượt `arg_traversal` — một cột trả lời về một thứ khác với thứ đang được hỏi. `empty` vẫn ở telemetry (`waf:v2:scan:empty`) và vẫn ra cột `scan=`; chỉ bỏ khỏi phát luật.
- 2026-09-25 — **`sev=critical` giờ cần cả năm điều kiện, không phải hai.** Bản trước tôi sửa `sev=` đọc `h.excepted` nhưng bỏ ba đường còn lại làm `action=block` không dẫn tới chặn thật: `h.mode == "enforce"` (luật không ở chế độ bóng), `dec.mode == "enforce"` (cả WAF không ở chế độ bóng), `dec.action == "block"` (phán quyết cuối đúng là chặn). Dữ liệu xác nhận: `corr_upload_php_payload` trên 171-96 có `wact=allow` mà đang mang `sev=critical`. Bất kỳ cảnh báo nào gác trên `sev=critical` sẽ kêu vì một luật **có ý** không chặn — và báo động giả làm người ta ngừng đọc báo động.
- 2026-09-25 — **`wafstat.sh`: `FP%` đổi tên thành `auth%`, và mục `0c` có ngưỡng dân số 30.** Hai nhãn của chính tôi gây hiểu sai, và dữ liệu thật cho thấy điều đó. (a) `FP%` chia cho số lượt của luật, nên với một luật **không chặn** nó là tỉ lệ *thành phần*, không phải tỉ lệ chặn sai; `body_scan_incomplete` ra `36.0%` trên 171-96 trong khi nó là luật `observe` `score=0` không chặn ai — con số đúng, tên cột sai, và sai theo hướng dễ gây hành động. Giờ chỉ dòng `wact=block` mới in `-> FP THẬT`. (b) Mục `0c` nói "0 auth — có thể xét enforce" cho ba luật có đúng **1 lượt** mỗi cái, và tôi đã đọc "0 auth" như một bằng chứng. Một lượt không phải bằng chứng, nó là một lần: với n nhỏ, "0 auth" và "chưa gặp ca auth nào" không phân biệt được. Ngưỡng 30 không thiêng — chỉ là chỗ dừng lại để không kết luận từ dân số một chữ số.
- 2026-09-25 — **Ba dòng bóng trên 171-96 là BA request khác nhau, cùng một IP — không phải một request như tôi đoán.** `rid=` khác nhau cả ba: `body_php_code`, `upload_php_ext`, `corr_upload_php_payload`, đều từ `107.161.175.27`. Tức một IP thử ba hình: gửi PHP code trong thân, gửi tên tệp `.php`, rồi lần thứ ba trúng cả hai nên correlation nổ. Hai luật đầu có `mode=enforce` mà vẫn `wact=allow` vì chúng là `action = "signal"`; `would=block` của chúng đến từ correlation. Dân số vẫn 3, vẫn quá nhỏ để promote, nhưng không còn là "một lần". **ĐÍNH CHÍNH 26-09: SAI — `rid=` không ổn định (xem mục `rid=` 26-09); ba dòng cùng `would=block` là MỘT request.**
- 2026-09-25 — **`spill` ĐÃ BỊ GỠ khỏi `arg_factor_body`: nó là điều kiện KẺ GỬI ĐIỀU KHIỂN ĐƯỢC.** Review chỉ ra lỗ này và nó đúng: `spill = true` chỉ có nghĩa "thân request đã ra file tạm vì vượt `client_body_buffer_size`" — chỉ cần nhồi padding cho một thân `urlencoded` hoặc JSON vượt buffer là mọi payload traversal trong đó **từ 35 điểm còn 1,75**. Trục 1 trong lập luận ban đầu ("tấn công tham số không cần megabyte") **đúng theo chiều quan sát nhưng sai theo chiều điều khiển** — kẻ tấn công không bị buộc phải giữ payload nhỏ, và một quan sát trên lưu lượng hiện tại không nói gì về thứ kẻ tấn công *có thể* làm. Đây là cùng họ lỗi với `0600` (đọc một tương quan như một nguyên nhân) nhưng nghiêm trọng hơn vì nó tạo ra một **cửa hạ điểm mở cho người ngoài**. Chín ca FP đo được đều là `multipart` **và** `spill`, nên gỡ `spill` không mất ca nào: `family == "multipart"` phủ đủ 9/9, và định dạng thì kẻ gửi không đổi được mà không đổi bản chất request (phải có boundary và part hợp lệ mới đến được đây). **Và cầu tương thích cũ giờ cũng áp `factor`** (`init.lua:record_arg`): trước đó nó nạp `rule.score` đủ vào `ctx.waf_arg`/`ctx.waf_body_arg`, nên một request policy đã hạ xuống 5% vẫn nạp đủ điểm vào đường chấm điểm cũ. Hiện không ai thấy vì hai tín hiệu đó ở trọng số 0 — đúng kiểu "hai đường, một đường không được cập nhật" đã mất bốn tháng ở `wp_paths.mark()`.
- 2026-09-25 — **Cấu hình từ chối KHÓA LẠ, ở mọi scope.** Validator cũ kiểm *giá trị* của khóa nó biết và **bỏ qua khóa lạ trong im lặng**. Bốn ca thật: `mod = "shadow"` → hệ thống giữ `enforce`; `mdoe` trong rule override → không hiệu lực; `wordpres = false` → profile WordPress vẫn bật; `uri_prefx = "/admin/"` → **exception mất ràng buộc URI và áp cho toàn bộ luật trên host**. Ba ca đầu làm cấu hình không được áp (an toàn hơn ý muốn, nhưng người vận hành tưởng đã áp); **ca thứ tư nguy hiểm theo chiều ngược** — một gõ sai chính tả mở một lỗ trên WAF. Nên: khóa lạ = LỖI, `configure()` trả `false` và giữ cấu hình cũ. Tập `profiles` hợp lệ lấy **từ registry** chứ không hardcode. `configure()` đã kiểm rule-id/family/correlation-id không tồn tại từ trước — cái thiếu là khóa *bên trong* override.
- 2026-09-25 — **`waf.log` giờ mang phán quyết THẬT của policy: 6 cột `mode= exc= wact= would= wscore= pver=`.** Trước đó dòng log chỉ có `action=` — hành động mà LUẬT muốn, một hằng số theo `rule_id`. Ba trường hợp khác hẳn nhau cho ra một dòng y hệt nhau: luật hard-block bị exception chặn, correlation đang ở chế độ bóng, và luật thật sự đã chặn. Nghĩa là **không đo được FP từ `waf.log`**: không phân biệt "sẽ chặn" với "đã chặn", không truy được exception nào suppress luật nào. Chế độ bóng mà không đọc được số liệu bóng thì không còn là chế độ bóng. Sáu cột này không phải đặc điểm mới — chúng là thứ policy **đã tính** và nằm sẵn trên từng `hit` (`policy.lua:140-155`); logger chỉ không in. Đồng thời `sev=` giờ đọc cả `h.excepted`: một luật hard-block bị exception chặn không còn là `critical`. `wafstat.sh` có hai mục mới (`0b`, `0c`) trả lời đúng câu hỏi của giai đoạn bóng: luật này **sẽ** chặn bao nhiêu, và trong đó bao nhiêu bắn vào phiên đăng nhập thật.
- 2026-09-25 — **Telemetry có ĐƯỜNG RA: `/antibot-admin/waf` + `telemetry.snapshot()`.** `waf:v2:*` được ghi từ đường request nhưng **không một nơi nào đọc lại** — số liệu vào rồi nằm đó, đúng họ lỗi "cảnh báo không đến ai" (kiểm ĐƯỜNG RA, không chỉ kiểm phát hiện). Thêm nữa `incr` bọc trong `pcall` rồi **bỏ kết quả**: khi `antibot_cache` đầy — nó dùng chung với dữ liệu vận hành khác — mọi phép ghi thất bại và bảng số liệu **đứng yên** thay vì giảm, đọc vào sẽ thấy "không có tấn công nào", giống hệt trường hợp không có tấn công. Nay có `waf:v2:write_errors`. `snapshot()` chỉ gọi từ admin (người vận hành bấm) vì `get_keys()` **khóa toàn bộ dict** trong lúc quét, và nó báo ra `truncated` khi chạm giới hạn 2048 khóa thay vì để người đọc tưởng đang xem toàn bộ.
- 2026-09-25 — **`body.probe` giờ chèn được qua `rt`, nên `_run_pre_with_runtime` mới thật sự xác định.** Helper nhận `rt` giả lập nhưng `init.lua:337` gọi `body.probe(ctx)` cứng, và `probe` đọc `ngx.var`/`ngx.req` từ global — nên với request **có thân**, test vẫn đọc global thật. `rt.waf_body_probe` là điểm chèn chỉ test đặt; production không đặt nên nhánh dưới chạy y như trước. Nhờ đó `policy_test.lua` kiểm được `arg_factor_body` trên đúng ba nhánh, gồm ca **`spill` một mình trên `urlencoded` phải KHÔNG giảm điểm** — chính cái lỗ vừa đóng.
- 2026-09-25 — **Cột `fim=` đổi nghĩa: "FIM có đánh dấu" thay vì "FIM đánh dấu VÀ thắng luật".** V1 chỉ đặt `ctx.waf_fim_new = b` khi `b > score` (FIM phải thắng điểm luật đường dẫn); V2 đặt khi `factor > 0`. `ctx.waf_wp_path` vẫn nhận cùng giá trị (`max_field` giữ max, y như `if < then` cũ), và `waf_fim_new` **không có trong `DEFAULT_WEIGHTS`** — chỉ `async/waf_logger.lua:358` đọc nó. Nên đây là thay đổi **telemetry thuần**, không đổi quyết định, và đổi theo hướng đúng: cột `fim=` giờ nói điều nó đáng lẽ phải nói. Đo trước khi đổi: `fim=1` là **4/17.816 dòng** trên ba máy (chỉ 186-126 có), vì `fim.sh` giờ chỉ báo 2 file và cả hai không ai gọi qua HTTP — kẻ tấn công dùng cron. Cầu `waf:fimnew:` đúng (hai đầu đều có: `fim.sh:1538` ghi `SETEX`, `init.lua` đọc), chỉ không có lưu lượng.
