...
| P7 | nguy hiểm | luật trong bất kỳ byte PHP thấy phải có ở ÍT NHẤT một vùng |
...
| P2 | thông tin | khoảng tệp của WAF vs `$_FILES` — lệch là FP-side (`filename=""` bị PHP bỏ, trùng tên field) |

```bash
bash tools/wafdiff/run.sh                 # trong WSL — N=1500 + quét cắt thân
N=3000 SEED=99 bash tools/wafdiff/run.sh  # mã thoát 1 = có vi phạm; ca lỗi ở /var/tmp/wafdiff
```

CHỈ chạy trên máy dev: nó gọi `php-cgi` với một script PHP, nên tự từ chối trên máy có DirectAdmin, không có file `.php` nào trong repo (oracle được sinh lúc chạy vào `/var/tmp`), và nằm ngoài `antibot-core/` nên `deploy.sh` không đưa nó vào cây của nginx. **Bắt buộc chạy trước mọi commit chạm `file_ranges`/`projection`/`scan_disposition_headers`/`php_filename`.** Công cụ đã được kiểm bằng đột biến: coi field là tệp → P1+P6, bỏ quét vùng tệp → P7, `pack` đánh rơi vùng tệp → P3.
