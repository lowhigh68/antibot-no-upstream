...
- 2026-09-11 — **`h2_order == nil` +0.1: GIỮ. Đóng case, và suýt sửa một lỗi không tồn tại.** Luật dừng chốt trước là "vài chục thì giữ, ~470 thì xoá", nhưng nó giả định nhóm `nil` đồng nhất. Số liệu bác bỏ đúng giả định đó: 18 / 72 / 148 / 660 / 3.086 trên 5 máy, chênh 170 lần. Thứ trả lời được là **thành phần**, không phải số đếm — nhóm "`h2_order=nil` **có xác thực**" gần như toàn bộ là **bot tự khai**: facebookexternalhit, meta-externalagent/externalads, Baiduspider(-render), AhrefsBot, ExaSearchBot, Kimi, Grok, Qwen, Brave, Hunyuan, YiBot, ClaudeBot, cộng `NetworkingExtension`/`com.apple.WebKit.Networking` (tầng mạng iOS/macOS, không phải trình duyệt). Nhánh đang cộng điểm **đúng đối tượng**.
  - **Ngoại lệ 3.086 của cloud183-139 là ảo.** 2.250 dòng mang hình dạng Android WebView, và máy đó chạy bản **trước `aca6bef`** nên cột `ua=` vẫn cắt ở 120 — `len=120` ở *mọi* dòng, thấy rõ `Chrome` bị cắt ngang token (`…Version/4.0_Chrome`, `…_Chrom`, `…_Chr`). Trên máy đã cập nhật chúng khớp `ua_chrome` và không bao giờ vào nhóm `nil`. Bỏ ra còn 836 (≈35,6/giờ), dẫn đầu `meta-externalads` 382 + `znetwork` 257 — cùng hình dạng 4 máy kia.
  - **KHÔNG thêm nhánh Android WebView vào `infer_from_ua`.** Đã dự phòng sẵn phương án đó và nó sai: các UA ấy *có* `Chrome/`. Thêm vào là sửa một lỗi không tồn tại dựa trên số liệu của một máy chạy mã cũ. Lần thứ sáu một kết luận bị cột log cắt cụt làm hỏng — lần đầu bắt được **trước** khi sửa mã.
  - Gốc không nằm trong Lua: **không có cách nào hỏi một máy đang chạy bản nào**. Vá ở `nginx/deploy.sh` bước `[4]` — ghi `$TARGET_DIR/VERSION` (sha, subject, ngày commit, ngày deploy, hostname) **sau** `rsync --delete`, biến câu hỏi đó thành một lệnh `cat`.
- 2026-09-11 — **Nửa HTTP/2: rà xong. Không có cửa thứ sáu; có một họ trình duyệt thật bị tính oan.**
  - **`h2_order` KHÔNG đo từ dây.** Nó là `infer_from_ua()` — tra một bảng 10 mẫu theo **User-Agent**, chuỗi do client tự chọn. Vậy nên `h2_sig` và một phần `h2_bot_confidence` phụ thuộc UA, không phải quan sát tầng vận chuyển. Không phải lỗi (repo đã ghi ở `core/fingerprint/build_light.lua`), nhưng **đừng đọc chúng như bằng chứng transport**.
  - **`h2_order == nil → +0.1` nghĩa là "UA không nằm trong bảng"**, ×55 = **5,5 điểm**. GIỮ LẠI, có số liệu: đo 09-11, 5 máy, 3,7 giờ, 107.256 request H2 → nhóm `nil` còn **9,8%** và gần như toàn crawler tự khai tên. Với chúng, 0,1 là đúng.
  - **Ca thứ BA của cùng một lỗi trong `pseudo_header.lua`** — sau Firefox (+13,75 oan) và client Go (+13,75 oan): một họ client thật không có trong bảng thì bị tính là đáng ngờ. Lần này là **WebView iOS** (Facebook/Instagram/Zalo), bỏ cả `Safari/` lẫn `Version/` nên rơi ra ngoài. Thêm mẫu `AppleWebKit/605 → mspa`. **KHÔNG** mở sang `AppleWebKit/537.36`: ClaudeBot, GPTBot, ChatGPT-User đều khai chuỗi đó mà không có `Chrome/`.
  - **CẢNH BÁO CHO NGƯỜI ĐO SAU:** bản đo đầu ra **19%** nil với **17.988** phiên đăng nhập. Cả hai **đều ảo** — cột `ua=` khi đó cắt ở 120 ký tự, mà `Safari/` của iPhone bắt đầu ở byte **123**. Production đọc `ctx.ua` đầy đủ nên vẫn nhận đúng. Đã sửa ở `aca6bef` (cắt 180). Đo lại trước khi tin số cũ.
  - **`ja3_cipher` vẫn `"probe"`, và đó là kết luận chứ không phải sự trì hoãn.** Nhóm `ja3c=100` (28.002 request, 5.911 IP) hoá ra là **Applebot**, 100% `good_bot_verified`/`good_bot_asn_lite` — nên `ja3c=100` KHÔNG phải trần cắt thư viện, và không còn ứng viên nào cho `ja3:block:`. Ngoài ra `ctx.ja3_partial` chặn **cả** `check_lists`, nên nếu sau này cần chặn theo vân tay TLS thì việc đúng là **mở cổng cho `check_lists` chạy trên hash partial** (hash trong log chính là hash đem so), KHÔNG phải lật `"on"` — lật là chịu churn danh tính toàn đàn cộng một pipeline Redis mỗi request.
- 2026-09-09 — **Trần log: đếm theo TỪNG LOẠI, không dùng chung một biến.**
  - **Đo được trong sản xuất, 5 máy, 22,7 giờ:** `cipher_invalid shape=nil` **28** lần, `shape=api_err` **12** lần (4/5 máy). Cửa thứ tư không phải lý thuyết — nó đang bắt lỗi API thật. Trước bản vá, 12 ca đó lẫn vào `shape=nil` và không phân biệt được với "không có dữ liệu".
  - **Một `_apierr_n` dùng chung hỏng theo ba cách**, không phải một: (1) loại ồn ào che hoàn toàn loại hiếm — đúng cặp 28/12 ở trên; (2) `lan thu N` in ra là **tổng lỗi của worker**, không phải lần thứ N của loại đang in, nên không suy ra tần suất được; (3) hai loại xen kẽ đều đặn thì một loại **không bao giờ** rơi trúng `% 200 == 1` — im lặng vĩnh viễn, không phải "lấy mẫu thưa". Nay `_errn[what]`.
  - **KHOÁ BUCKET PHẢI LÀ TẬP ĐÓNG.** `sv_err` chứa `"di dang khai bao=<N> than=<M>"` với N, M do client quyết định — dùng làm khoá thì bảng phình theo dữ liệu người lạ gửi, đúng cái bẫy ghi Redis không chặn của `wp_paths`. Chi tiết đi vào **thông điệp**, không đi vào khoá. Tách bucket chỉ khi hai thứ là hai **nguyên nhân** khác nhau (`nil` vs `api_err`), không phải hai biến thể của cùng một sự kiện.
  - **Chín nhánh ERR trong đường bắt tay nay đều có trần**, không chỉ nhánh cipher. Hai nhánh nguy hiểm nhất không cần sự cố gì: `ext_present_hash` và `ext_present_unavail` là **thuộc tính phiên bản thư viện** — trên lua-resty-core cũ chúng ghi đúng một dòng **mỗi bắt tay**. `ext_not_ok` và `cipher_too_few` chỉ sống dậy khi `CIPHER_MODE == "on"`, và tỉ lệ `ext_ok=false` **không đo được ở nấc `probe`** (không cột log nào chở nó) — nên phải có trần **trước** khi lật, nếu không ngày lật là ngày `error.log` không đọc được nữa.
  - **CÒN HỞ (cố ý không sửa):** `capture_miss reason=set_failed` vẫn không có trần. Họ khác (ghi shared dict, không phải đọc API), nhưng cũng ghi mỗi bắt tay khi `antibot_tls` đầy.
  - `contract_test` 14d **đổi từ regex sang hành vi**: `_bump(tbl, what)` là hàm thuần, test bơm 400 lỗi **xen kẽ hai loại** và đòi mỗi loại in đúng 2 dòng. Counter dùng chung cho ra 4/0 — tức bản cũ xanh với **cả hai** cài đặt, còn bản này thì không.
- 2026-09-08 — **Cửa thứ tư của cùng một con bug: RANH GIỚI API.**
  ...
  - **Hai hàm thuần `api_state` / `read_ext_list`** tách quyết định ra khỏi `ngx`, nên `contract_test` mục 14 nay kiểm **hành vi** chứ không chỉ tìm chuỗi: bốn trạng thái vào, hai quyết định `ext_ok` ra. Bản đầu chỉ kiểm nguồn — xoá `ext_ok = false` mà giữ dòng log thì test vẫn xanh. Vẫn giữ lớp kiểm nguồn song song, vì hàm thuần có đúng đến mấy cũng vô nghĩa nếu `err` không bao giờ tới được nó.
  - **Log lỗi API có trần 1/200 mỗi worker** (`log_api_err`, in kèm số đếm thật). Một bản thư viện hỏng hệ thống sẽ sinh một dòng ERR **mỗi bắt tay TLS** — tự tay giết chính file dùng để chẩn đoán. Nhánh cipher trước ghi hai dòng cho một sự kiện (`... API loi` rồi `cipher_invalid shape=api_err`); nay nguyên nhân đi kèm vào chính dòng `cipher_invalid`, giữ nguyên token đó vì `do_sang.sh` đang đếm.
